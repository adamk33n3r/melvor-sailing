...
class FarmingPlot extends NamespacedObject {
  ...
  encode(writer) {
    ...
    if (this.plantedRecipe !== undefined) writer.writeNamespacedObject(this.plantedRecipe);
    writer.writeBoolean(this.compostItem !== undefined);
    if (this.compostItem !== undefined) writer.writeNamespacedObject(this.compostItem);
...
class Farming extends SkillWithMastery {
  ...
  queueBankQuantityRender(item) {
    if (item instanceof CompostItem) this.renderQueue.compostQuantity = true;
    this.renderQueue.plotUnlockQuantities = true;
  }
  queueCurrencyQuantityRender(currency) {
    if (currency instanceof GP) this.renderQueue.plotUnlockQuantities = true;
  }
  getErrorLog() {
    const plotLog = [];
    this.plots.forEach((plot) => {
...
${plotLog.join('\n')}`;
  ...
  growPlots(timer) {
    ...
    timer.plots.forEach((plot) => {
      ...
      this.game.stats.Farming.add(
        success ? FarmingStats.TimeSpentWaitingForCrops : FarmingStats.TimeSpentWaitingForDeadCrops,
        plot.growthTime,
      );
      if (!success) this.game.stats.Farming.inc(FarmingStats.CropsDied);
  ...
  applyPrimaryProductMultipliers(item, quantity, action, query) {
    ...
    if (action instanceof FarmingRecipe) quantity *= action.category.harvestMultiplier;
    return quantity;
  }
  harvestPlot(plot) {
    ...
    if (recipe.category.scaleXPWithQuantity) xpToAdd *= harvestQuantity;
    rewards.addXP(this, xpToAdd, recipe);
    let abyssalXPToAdd = recipe.baseAbyssalExperience;
    if (recipe.category.scaleXPWithQuantity) abyssalXPToAdd *= harvestQuantity;
  ...
  plantPlot(plot, recipe, isSelected = false) {
    ...
    if (recipe.category.giveXPOnPlant) this.addXP(recipe.baseExperience, recipe);
  ...
  passiveTick() {
    if (this.growthTimers.size > 0) {
      this.growthTimers.forEach((timer) => {
        timer.tick();
        if (timer.ticksLeft % TICKS_PER_MINUTE === 0 && timer.ticksLeft > 0) this.renderQueue.growthTime.add(timer);
      });
    }
  }
  onUnlock() {
  ...
  renderGrants() {
    if (this.renderQueue.grants.size === 0) return;
  ...
  renderGrowthStatus() {
    if (this.renderQueue.growthTime.size === 0) return;
  ...
  renderGrowthState() {
    if (this.renderQueue.growthState.size === 0) return;
  ...
  renderGrowthChance() {
    if (this.renderQueue.growthChance.size === 0) return;
  ...
  renderCompost() {
    if (this.renderQueue.compost.size === 0) return;
  ...
  renderSelectedSeed() {
    if (this.renderQueue.selectedSeed.size === 0) return;
  ...
  renderGrowthIndicators() {
    if (!this.renderQueue.growthIndicators) return;
    let anyGrown = false;
    this.categories.forEach((category) => {
      var _a;
      const plots = this.getPlotsForCategory(category);
  ...
  renderCompostQuantity() {
    if (!this.renderQueue.compostQuantity) return;
  ...
  renderPlotVisibility() {
    if (!this.renderQueue.plotVisibility) return;
  ...
  renderPlotUnlockQuantities() {
    if (!this.renderQueue.plotUnlockQuantities) return;
  ...
  compostPlot(plot, compost, amount) {
    ...
    if (plot.compostItem !== undefined && plot.compostItem.harvestBonus >= compost.harvestBonus) {
      ...
      if (amount > 0) {
        ...
        if (!freeCompost) this.game.bank.removeItemQuantity(compost, amount, true);
  ...
  compostAllOnClick(category, compost) {
    ...
    const plots = this.getPlotsForCategory(category);
    plots.forEach((plot) => {
      if (!this.game.bank.hasItem(compost)) return;
      if (this.compostPlot(plot, compost, Infinity)) appliedAny = true;
    });
    if (appliedAny) {
  ...
  destroyPlot(plot) {
    ...
    if (plotIndex === -1) throw new Error('Tried destroying plot, but plot is not a member of timer.');
    this.growthTimerMap.delete(plot);
  ...
  getPkgObjects(pkg, type) {
    var _a, _b;
    const skillData =
      (_b = (_a = pkg.data) === null || _a === void 0 ? void 0 : _a.skillData) === null || _b === void 0
        ? void 0
        : _b.find(({ skillID }) => skillID === this.id);
  ...
  decodePlot(reader, version) {
    ...
    if (typeof plot === 'string') {
      if (plot.startsWith('melvor')) plot = this.plots.getDummyObject(plot, DummyFarmingPlot, this.game);
      else plot = this.game.constructDummyObject(plot, DummyFarmingPlot);
    }
    ...
    if (save.newFarmingAreas !== undefined) {
      save.newFarmingAreas.forEach((area, catID) => {
        area.patches.forEach((patch, i) => {
          ...
          if (patch.compost > 0) plot.compostItem = patch.gloop ? gloop : compost;
          if (patch.unlocked) {
            if (patch.seedID > 0) {
    ...
    if (save.plantAllSelected !== undefined) {
      Object.entries(save.plantAllSelected).forEach(([catID, seeds]) => {
        seeds.forEach((seedID, i) => {
          ...
          if (recipe !== undefined) plot.selectedRecipe = recipe;
        });
      });
    }
  }
