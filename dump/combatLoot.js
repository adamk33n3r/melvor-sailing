...
class CombatLoot {
  ...
  add(item, quantity, stack = false) {
    if (stack) {
      ...
      this.makeDropRoom();
      let startIndex = 1;
      if (this.game.modifiers.autoBurying > 0 && this.game.prayer.isUnlocked) startIndex = 0;
  ...
  makeDropRoom() {
    ...
    if (this.drops.length === this.maxLoot) {
      ...
      if (removed !== undefined) {
        this.lostLoot.set(
          removed.item,
          ((_a = this.lostLoot.get(removed.item)) !== null && _a !== void 0 ? _a : 0) + removed.quantity,
        );
      }
    }
  }
  lootAll() {
    this.drops = this.drops.filter((drop) => {
      const fit = this.game.bank.addItem(
        drop.item,
        drop.quantity,
        false,
        true,
        false,
        true,
        'Other.CombatLootContainer',
      );
      if (fit) this.game.stats.Combat.add(CombatStats.ItemsLooted, drop.quantity);
      return !fit;
    });
  ...
  deserialize(reader, version, idMap) {
    ...
    if (numDrops > this.maxLoot) throw new Error('Number of drops exceeds maximum drops.');
...
class CombatLootMenuElement extends HTMLElement {
  ...
  createDropElement() {
    ...
    this.dropElements.push({ container: drop, image: dropImage, qty: dropQty, tooltip: tooltip });
  }
