...
class EquipmentSlot extends NamespacedObject {
  constructor(namespace, data, game) {
    ...
    try {
      ...
      if (data.requirements !== undefined) game.queueForSoftDependencyReg(data, this);
    } catch (e) {
  ...
  registerSoftDependencies(data, game) {
    try {
      if (data.requirements !== undefined) this.requirements = game.getRequirementsFromData(data.requirements);
    } catch (e) {
      throw new DataConstructionError(EquipmentSlot.name, e, this.id);
    }
  }
  applyDataModification(modData, game) {
    if (modData.allowQuantity !== undefined) this.allowQuantity = modData.allowQuantity;
    if (modData.providesEquipStats !== undefined) this.providesEquipStats = modData.providesEquipStats;
    if (modData.gridPosition !== undefined) {
      ...
      if (EquipmentSlot.isGridPositionFree(modData.gridPosition)) {
        ...
        console.warn(
          `Warning modifying Equipment Slot: ${this.id}. Grid Position: ${modData.gridPosition.col}, ${modData.gridPosition.row} is occupied.`,
        );
      }
    }
  ...
  static getValidGridPosition(data) {
    ...
    if (this.isGridPositionFree(data.gridPosition)) return data.gridPosition;
    if (data.alternativePositions !== undefined) {
      for (let i = 0; i < data.alternativePositions.length; i++) {
        ...
        if (this.isGridPositionFree(position)) return position;
      }
    }
  ...
  static setGridPosition(position) {
    ...
    gridRow.add(col);
    if (row < this.gridRowRange[0]) this.gridRowRange[0] = row;
    if (row > this.gridRowRange[1]) this.gridRowRange[1] = row;
  }
  static freeGridPosition(position) {
    const { row, col } = position;
    const gridRow = this.gridPositions.get(row);
    if (gridRow === undefined) throw new Error(`Error freeing grid position, position is not set.`);
    gridRow.delete(col);
    if (gridRow.size === 0) {
  ...
  static getGridSize() {
    return {
      cols: { min: this.gridColRange[0], max: this.gridColRange[1] },
      rows: { min: this.gridRowRange[0], max: this.gridRowRange[1] },
    };
  }
}
...
EquipmentSlot.SUMMONING_SYNERGY_POSITION = { col: 2, row: 5 };
EquipmentSlot.init();
...
class Equipment {
  ...
  get isWeapon2H() {
    return this.equippedItems['melvorD:Weapon'].item.occupiesSlots.some((slot) => slot.id === 'melvorD:Shield');
  }
  get equipMenuMedia() {
    return this.equippedItems['melvorD:Helmet'].media;
  }
  getItemsAddedOnEquip(item, slot) {
    if (slot === undefined) slot = item.validSlots[0];
    ...
    const itemsAdded = slotsToUnequip.map((slot) => {
      ...
      return { item: equippedItem.item, quantity: equippedItem.quantity };
    });
    return itemsAdded;
  }
  getItemsAddedOnUnequip(slot) {
    ...
    return { item: unequipSlot.item, quantity: unequipSlot.quantity };
  }
  getSlotsToUnequip(itemToEquip, slot) {
    ...
    slotsRequired.forEach((slot) => {
      ...
      if (!equippedItem.isEmpty) {
        ...
        if (!slotsToUnequip.includes(rootSlotType)) slotsToUnequip.push(rootSlotType);
      }
    });
  ...
  equipItem(itemToEquip, slot, quantity) {
    ...
    if (itemToEquip.consumesChargesOn !== undefined) this.itemChargeUsers.add(this.equippedItems[slot.id]);
  ...
  unequipItem(slot) {
    ...
    if (unequip.item.consumesChargesOn !== undefined) this.itemChargeUsers.delete(unequip);
  ...
  checkForItemIDs(itemIDs) {
    ...
    return this.equippedArray.some((equippedItem) => {
      ...
      if (idIndex !== -1) idsNotFound.splice(idIndex, 1);
  ...
  getSnapshot() {
    ...
    this.equippedArray.forEach((equippedItem) => {
      if (equippedItem.slot.allowQuantity)
        snapshot.set(equippedItem.slot, { item: equippedItem.item, quantity: equippedItem.quantity });
    });
    return snapshot;
  }
  encode(writer) {
  ...
  deserialize(reader, version, idMap, addOnFail = false) {
    ...
    for (let i = 0; i < numItems; i++) {
      ...
      if (item !== undefined && slot !== undefined) {
        this.equipItem(item, slot, quantity);
        if (version < 21 && item.fitsInSlot(slot.id)) this.equippedItems[slot.id].quickEquipItems[0] = item;
      } else if (addOnFail) this.game.bank.addDummyItemOnLoad(newID, quantity);
    }
  }
  convertFromOldFormat(oldData, idMap) {
    ...
    oldData.equipment.forEach((oldItemID, oldSlotID) => {
      if (oldItemID !== 0) {
        let quantity = 1;
        if (oldSlotID === OldEquipmentSlotIDs.Quiver) quantity = oldData.ammo;
        if (oldSlotID === OldEquipmentSlotIDs.Summon1) quantity = oldData.summonAmmo[0];
        if (oldSlotID === OldEquipmentSlotIDs.Summon2) quantity = oldData.summonAmmo[1];
  ...
  static getEquipStatDescription(stat) {
    ...
    if (stat.value > 0) statValue = '+' + statValue;
  ...
  static getSummoningMaxHitStatDescription(damageType, value) {
    ...
    if (value > 0) statValue = '+' + statValue;
...
class EquipmentSetMenu {
  constructor(containerID, buttonClasses) {
    ...
    if (cont === null) throw new Error(`Invalid id: ${containerID}`);
    this.container = cont;
  }
  ...
  addButton(text) {
    const newButton = createElement('button', { text: text, classList: [...this.buttonClasses, 'btn-primary'] });
    newButton.style.pointerEvents = 'auto';
    const newTooltip = this.createTooltip(newButton);
    const button = { button: newButton, tooltip: newTooltip };
    this.buttons.push(button);
    this.container.append(newButton);
    return button;
  }
  createTooltip(parent) {
    return tippy(parent, { content: '', allowHTML: true, placement: 'top', interactive: false, animation: false });
  }
}
class EquippedItem {
  ...
  isItem(item) {
    if (typeof item !== 'string') return item === this.item;
    else {
      switch (item) {
        case 'ThrowingWeapon':
          return (
            this.item instanceof WeaponItem &&
        ...
        case 'Melee2HWeapon':
          return (
            ...
            this.item.occupiesSlot('melvorD:Shield')
          );
        default:
          return false;
      }
  ...
  setQuickEquip(pos) {
    if (this.isEmpty || this.item.fitsInSlot(this.slot.id)) this.quickEquipItems[pos] = this.item;
  }
  trimQuickEquipItems() {
...
class EquipmentGridIconElement extends HTMLElement {
  ...
  connectedCallback() {
    ...
    if (!this.quickEquipEnabled) this.quickEquipTooltip.disable();
  }
  ...
  setSlot(slot, game) {
    ...
    if (slot.allowQuantity) showElement(this.quantity);
    else hideElement(this.quantity);
  ...
  setEquipped(player, equipped) {
    ...
    if (this.quickEquipTooltip !== undefined) {
      this.quickEquipTooltip.setProps({ onShow: () => this.quickEquip.setEquipped(player, equipped) });
    }
  }
  setQuickEquipEnabled(isEnabled) {
    if (this.quickEquipEnabled === isEnabled) return;
    if (this.quickEquipTooltip !== undefined) {
      if (isEnabled) this.quickEquipTooltip.enable();
      else this.quickEquipTooltip.disable();
    }
  ...
  getSynergyTooltipContent(synergyDescription) {
    ...
    switch (synergyDescription) {
      case '':
        ...
        break;
      case 'Locked':
        html += `font-size-sm text-danger">${getLangString(
          'COMBAT_MISC_118',
        )}</h5><h5 class="font-w400 font-size-sm text-warning mb-0">${getLangString('COMBAT_MISC_119')}`;
        break;
      default:
        html += `text-success">${getLangString(
          'COMBAT_MISC_120',
        )}</h5><h5 class="font-w400 font-size-sm text-info mb-0">${synergyDescription}`;
        break;
    }
    html += `</h5></div>`;
    return html;
...
class EquipmentTooltipElement extends HTMLElement {
  ...
  setItem(item, showStats) {
    if (this.lastItem === item && !this.unset) return;
  ...
  setEmpty() {
    if (this.lastItem === undefined && !this.unset) return;
...
class QuickEquipTooltipElement extends HTMLElement {
  ...
  setEquipped(player, equipped) {
    ...
    equipped.quickEquipItems.forEach((item, i) => {
      ...
      if (!equipped.isEmpty && item === equipped.item) equip.classList.add('btn-outline-success');
      else equip.classList.add('btn-outline-secondary');
      set.onclick = () => {
        const changed = player.setQuickEquipItem(slot, i);
        if (changed) image.src = equipped.item.media;
      };
