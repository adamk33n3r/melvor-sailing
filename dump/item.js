...
class Item extends NamespacedObject {
  constructor(namespace, data, game) {
    ...
    try {
      ...
      this.sellsFor = { currency, quantity: data.sellsFor };
      if (data.isArtefact !== undefined) this._isArtefact = data.isArtefact;
      if (data.isGenericArtefact !== undefined) this._isGenericArtefact = data.isGenericArtefact;
  ...
  get modifiedDescription() {
    if (this._modifiedDescription !== undefined) return this._modifiedDescription;
  ...
  get artefactSizeAndLocation() {
    ...
    if (this.isArtefact) {
      const typeLocation =
        (_a = game.archaeology) === null || _a === void 0 ? void 0 : _a.getArtefactTypeAndLocationFromCache(this);
...
class EquipmentItem extends Item {
  constructor(namespace, data, game) {
    ...
    try {
      ...
      if (data.overrideSpecialChances !== undefined) this.overrideSpecialChances = data.overrideSpecialChances;
      if (data.providedRunes !== undefined) this.providedRunes = game.items.getQuantities(data.providedRunes);
      if (data.ammoType !== undefined) this.ammoType = AmmoTypeID[data.ammoType];
      if (data.deathPenaltyPriority !== undefined) this.deathPenaltyPriority = data.deathPenaltyPriority;
  ...
  get description() {
    ...
    if (StatObject.hasStats(this)) {
      if (this.fitsInSlot('melvorD:Passive')) {
  ...
  registerSoftDependencies(data, game) {
    try {
      ...
      if (data.modifiers !== undefined) this.modifiers = game.getModifierValuesFromData(data.modifiers);
    ...
    try {
      ...
      if (modData.conditionalModifiers !== undefined) {
        ...
        if (modData.conditionalModifiers.add !== undefined) {
          this.conditionalModifiers.push(
            ...modData.conditionalModifiers.add.map((data) => new ConditionalModifier(data, game, this)),
          );
        }
      }
      ...
      if (modData.equipRequirements !== undefined) {
        if (modData.equipRequirements.remove !== undefined) {
          modData.equipRequirements.remove.forEach(
            (type) => (this.equipRequirements = this.equipRequirements.filter((r) => r.type !== type)),
          );
        }
      ...
      if (modData.specialAttacks !== undefined) {
        if (this.overrideSpecialChances !== undefined && modData.overrideSpecialChances === undefined) {
          throw new Error(
            `An item with existing special chance overrides must provide new overrides when adding or removing special attacks.`,
          );
        }
        if (modData.specialAttacks.remove !== undefined) {
          modData.specialAttacks.remove.forEach(
            (id) => (this.specialAttacks = this.specialAttacks.filter((s) => s.id !== id)),
          );
        }
      ...
      if (modData.combatEffects !== undefined) {
        if (this.combatEffects === undefined) this.combatEffects = [];
      ...
      if (modData.consumesOn !== undefined) {
        if (modData.consumesOn.remove !== undefined) {
          modData.consumesOn.remove.forEach((type) => {
            if (this.consumesOn !== undefined) this.consumesOn = this.consumesOn.filter((t) => t.type !== type);
          });
        }
        if (modData.consumesOn.add !== undefined) {
          if (this.consumesOn === undefined) this.consumesOn = [];
...
class WeaponItem extends EquipmentItem {
  constructor(namespace, itemData, game) {
    ...
    try {
      ...
      if (itemData.ammoTypeRequired !== undefined) this.ammoTypeRequired = AmmoTypeID[itemData.ammoTypeRequired];
...
class PotionItem extends Item {
  ...
  set recipe(value) {
    if (value === this._recipe) return;
    if (this._recipe !== undefined) {
      this.modQuery.remove({ action: this._recipe, realm: this._recipe.realm });
    }
    if (value !== undefined) {
      this.modQuery.add({ action: value, realm: value.realm });
    }
    this._recipe = value;
  }
  get hasDescription() {
...
class ReadableItem extends Item {
  ...
  showContents() {
    if (this.modalID !== undefined) {
      ...
      else console.warn(`Tried to read item with id: ${this.id}, but modal with id: ${this.modalID} is not in DOM.`);
...
class OpenableItem extends Item {
  constructor(namespace, itemData, game) {
    ...
    try {
      ...
      if (itemData.keyItem !== undefined) this.keyItem = game.items.getQuantity(itemData.keyItem);
    } catch (e) {
...
class TokenItem extends Item {
  ...
  getTimesClaimed() {
    const tokensClaimed =
      this.game.stats.itemFindCount(this) -
...
class MasteryTokenItem extends Item {
  constructor(namespace, data, game) {
    ...
    try {
      ...
      if (data.rollInSkill !== undefined) this.rollInSkill = data.rollInSkill;
    } catch (e) {
  ...
  get description() {
    ...
    return templateLangString('MODIFIER_DATA_masteryToken', { value: this.percent.toFixed(1) });
  }
}
class CompostItem extends Item {
  constructor(namespace, itemData, game) {
    ...
    try {
      ...
      if (itemData.disableSeedRefund !== undefined) this.disableSeedRefund = itemData.disableSeedRefund;
      if (itemData.compostAllCost !== undefined)
        this.compostAllCost = game.getCurrencyQuantity(itemData.compostAllCost);
      else this.compostAllCost = { currency: game.gp, quantity: 2000 };
    } catch (e) {
...
class RuneItem extends Item {
  constructor(namespace, data, game) {
    ...
    this.subRunes = [];
    try {
      if (data.subRunes !== undefined) this.subRunes = game.items.runes.getArrayFromIds(data.subRunes);
      if (data.realm !== undefined) this.realm = game.realms.getObjectSafe(data.realm);
      else this.realm = game.defaultRealm;
    } catch (e) {
...
function getItemSpecialAttackInformation(item) {
  ...
  if (item.specialAttacks.length > 0) {
    item.specialAttacks.forEach((attack, id) => {
      ...
      spec += `<h5 class="font-size-sm font-w700 text-danger mb-0"><img class="skill-icon-xxs mr-1" src="${assets.getURI(
        'assets/media/main/special_attack.png',
      )}"><small>${attack.name} (${formatPercent(
        chance,
      )})</small></h5><h5 class="font-size-sm font-w400 text-warning mb-0"><small>${
        attack.modifiedDescription
      }</small></h5>`;
    });
  }
  return spec;
...
function createItemInformationTooltip(item, showStats = false) {
  ...
  if (item instanceof EquipmentItem) {
    ...
    if (item.fitsInSlot('melvorD:Consumable'))
      consumable += `<h5 class="font-w400 font-size-sm text-center m-1 mb-1"><small class="badge bg-primary">${getLangString(
        'EQUIP_SLOT_Consumable',
      )}</small></h5>`;
  }
  ...
  if (item instanceof FoodItem)
    ...
    }'><span class='text-success'>+${game.combat.player.getFoodHealing(item)}</span>`;
  ...
  if (upgrades !== undefined) {
    upgradeable = `<br><img class="skill-icon-xs mr-1" src="${assets.getURI(
      `assets/media/main/${upgrades[0].isDowngrade ? 'downgrade' : 'upgrade'}.svg`,
    )}"><small class="text-success">${
      upgrades[0].isDowngrade
        ? getLangString('MENU_TEXT_CAN_BE_DOWNGRADED')
        : getLangString('MENU_TEXT_CAN_BE_UPGRADED')
    }</small>`;
  }
...
function getAmmoTypeDescription(item) {
  if (item.ammoType === undefined || item instanceof WeaponItem) return '';
  return `<li><span class="font-size-xs badge-pill bg-primary">${getLangString(
    `AMMO_TYPE_${item.ammoType}`,
  )}</span></li>`;
}
function getAmmoTypeRequiredDescription(item) {
  if (item.ammoTypeRequired === undefined || item.ammoType !== undefined) return '';
  return `<li><span class="font-size-xs badge-pill bg-secondary text-white">${getLangString(
    'MENU_TEXT_REQUIRES',
  )} ${getLangString(`AMMO_TYPE_${item.ammoTypeRequired}_PLURAL`)}</span></li>`;
}
function getItemBaseStatsBreakdown(item) {
  ...
  item.equipmentStats.forEach((equipStat) => {
    ...
    html += `<h5 class="font-size-sm ${
      isPositive ? 'text-success font-w400' : 'text-danger'
    } mb-0"><small>${statDesc}</small></h5>`;
  });
  return html;
}
