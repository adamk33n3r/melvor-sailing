...
function generateGaussianNumber($mean, $stdDev) {
  ...
  const $randNumNorm =
    Math.sqrt(-2.0 * Math.log($randNumA)) *
...
function getExclusiveRandomArrayElements(array, numElements) {
  if (numElements > array.length) throw new Error('Cannot get more elements than length of array');
...
function addSetToSet(addTo, from) {
  for (const elem of from) addTo.add(elem);
}
...
function getStandardDescTextClass(description, fontWeight) {
  if (description.isDisabled) return 'text-disabled';
  if (game.settings.showNeutralAttackModifiers) return fontWeight ? 'font-w700 text-warning' : 'text-warning';
...
const plainDescriptionFormatter = (desc) => {
  if (desc.isDisabled) return `<span class="text-disabled">${desc.text}</span>`;
  return desc.text;
};
const getElementHTMLDescriptionFormatter =
  (tagName, className, fontWeight = true) =>
  (desc) => {
    let textClass = getStandardDescTextClass(desc, fontWeight);
    if (className !== undefined) textClass = `${className} ${textClass}`;
...
const getElementDescriptionFormatter =
  (tagName, className, fontWeight = true) =>
  (desc) => {
    let textClass = getStandardDescTextClass(desc, fontWeight);
    if (className !== undefined) textClass = `${className} ${textClass}`;
...
function createElement(tagName, options = {}) {
  ...
  if (options.innerHTML !== undefined) elem.innerHTML = options.innerHTML;
  if (options.children !== undefined) elem.append(...options.children);
  if (options.attributes !== undefined) options.attributes.forEach(([name, value]) => elem.setAttribute(name, value));
  if (options.id !== undefined) elem.id = options.id;
  if (options.parent !== undefined) options.parent.appendChild(elem);
  return elem;
}
...
function imageNotify(media, message, messageTheme = 'success') {
  fireBottomToast(
    `<div class="text-center"><img class="notification-img" src="${media}"><span class="badge badge-${messageTheme}">${message}</span></div>`,
  );
}
...
function processScheduledItemNotifications() {
  if (!isItemNotificationScheduled) return;
  const reducedArray = [];
...
function processItemNotify(item, quantity) {
  let access = '';
  if (game.settings.enableAccessibility) access = ` ${item.name}`;
  let qtyInBank = '';
  ...
  if (!game.settings.useLegacyNotifications) game.notifications.createItemNotification(item, quantity);
  else imageNotify(item.media, `${textSymbol}${numberWithCommas(quantity)}${qtyInBank}${access}`, textClass);
}
...
function stunNotify(damage) {
  fireBottomToast(
    `<div class="text-center"><img class="notification-img" src="${assets.getURI(
      'assets/media/skills/thieving/thieving.png',
    )}"><span class="badge badge-warning">${getLangString(
      'TOASTS_STUNNED',
    )} </span> <span class="badge badge-danger"> ${templateLangString('TOASTS_MINUS_HP', {
      damage: `${damage}`,
    })}</span></div>`,
  );
}
function bankFullNotify() {
  ...
  else imageNotify(assets.getURI('assets/media/main/bank_header.png'), getLangString('TOASTS_FULL_BANK'), 'danger');
}
function levelUpNotify(skill) {
  fireTopToast(
    `<div class="block block-rounded-double bg-dark p-2">
...
function notifyPreserve(skill) {
  if (game.settings.showItemPreservationNotifications) notifyPlayer(skill, getLangString('MISC_STRING_2'), 'info', 1);
}
function notifyPlayer(skill, message, messageTheme = 'success', quantity = 1) {
  let img = '';
  if (skill === -1) img = assets.getURI('assets/media/main/xmas_present.png');
  else img = skill.media;
  if (!game.settings.useLegacyNotifications) {
  ...
  } else imageNotify(img, message, messageTheme);
}
...
function currencyNotify(currency, amount) {
  switch (currency.id) {
    case 'melvorD:GP':
      if (!game.settings.useLegacyNotifications) game.notifications.createGPNotification(amount);
      else
        imageNotify(
          assets.getURI('assets/media/main/coins.png'),
          `${amount >= 0 ? '+' : ''}${formatNumber(amount)}`,
          amount >= 0 ? 'success' : 'danger',
        );
      break;
    case 'melvorD:SlayerCoins':
      if (!game.settings.useLegacyNotifications) game.notifications.createSlayerCoinsNotification(amount);
      else
        imageNotify(
          assets.getURI('assets/media/main/slayer_coins.png'),
          `${amount >= 0 ? '+' : ''}${formatNumber(amount)}`,
          amount >= 0 ? 'success' : 'danger',
        );
      break;
...
function notifyMasteryLevelUp(action, newLevel) {
  if (!game.settings.useLegacyNotifications) game.notifications.createMasteryLevelNotification(action, newLevel);
  else {
    fireBottomToast(
      `<div class="text-center"><img class="notification-img" src="${
        action.media
      }"><img class="skill-icon-xs" src="${assets.getURI(
        'assets/media/main/mastery_header.png',
      )}"><span class="badge badge-success">${newLevel}</span></div>`,
    );
  }
}
...
function notifyCompletionBaseGame() {
  let html = `<h5 class="font-w600 text-success">${getLangString(
    'MENU_TEXT_ACHIEVED_COMPLETION',
  )}</h5><h5 class="font-w400 font-size-sm">${getLangString(
    'MENU_TEXT_COMPLETION_MESSAGE',
  )}</h5><h5 class="font-w400 font-size-sm">${getLangString('MENU_TEXT_COMPLETION_BUY_CAPE')}</h5>`;
...
function notifyCompletionExpansion(namespace) {
  ...
  let html = `<h5 class="font-w600 text-success">${getLangString(
    titleLang,
  )}</h5><h5 class="font-w400 font-size-sm">${getLangString('MENU_TEXT_COMPLETION_MESSAGE_TOTH')}</h5>`;
  const stat = game.stats.General.get(GeneralStats.AccountCreationDate);
  if (stat === 0) return;
  html += `<h5 class="font-w400 font-size-sm">${templateLangString('COMPLETION_CHARACTER_AGE', {
    localisedAge: formatAsTimePeriod(new Date().getTime() - stat),
...
function notifyCompletionEverything() {
  let html = `<h5 class="font-w600 text-success">${getLangString(
    'MENU_TEXT_ACHIEVED_COMPLETION_ALL',
  )}</h5><h5 class="font-w400 font-size-sm">${getLangString(
    'MENU_TEXT_COMPLETION_MESSAGE_ALL',
  )}</h5><h5 class="font-w400 font-size-sm">${getLangString('MENU_TEXT_COMPLETION_BUY_CAPE_TOTH')}</h5>`;
  const stat = game.stats.General.get(GeneralStats.AccountCreationDate);
  if (stat === 0) return;
  html += `<h5 class="font-w400 font-size-sm">${templateLangString('COMPLETION_CHARACTER_AGE', {
    localisedAge: formatAsTimePeriod(new Date().getTime() - stat),
...
class ExperienceCalculator {
  ...
  levelToXP(level) {
    if (this.table.length >= level) return this.table[level - 1];
    else {
...
function printAbyssalXPTable(upToLevel = 60) {
  let a = ``;
  for (let i = 1; i <= upToLevel; i++) a += `${i}\t${numberWithCommas(abyssalExp.levelToXP(i))}\n`;
  console.log(a);
}
function printXPTable(upToLevel = 120) {
  let a = ``;
  for (let i = 1; i <= upToLevel; i++) a += `${i}\t${numberWithCommas(exp.levelToXP(i))}\n`;
  console.log(a);
}
...
function showStunnedNotification() {
  if (!game.settings.showCombatStunNotifications) return;
  fireTopToast(
    `<div class="block block-rounded-double bg-dark p-2">
  <div class="media d-flex align-items-center push">
...
function showSleepNotification() {
  if (!game.settings.showCombatSleepNotifications) return;
  fireTopToast(
    `<div class="block block-rounded-double bg-dark p-2">
  <div class="media d-flex align-items-center push">
...
function convertNameValuePairToMap(pairs) {
  const map = new Map();
  pairs.forEach((pair) => {
    if (map.get(pair.name) !== undefined) console.warn(`Duplicate pair found: ${pair.name}`);
...
function animateProgress(div, interval, stayFull = true) {
  ...
  if (stayFull) $(`#${div}`).css('-webkit-animation-fill-mode', 'both');
}
      ...
      if (typeof node === 'string') {
        ...
        if (portions.length > 0) {
          portions.forEach((stringPort, i) => {
            ...
            if (i !== portions.length - 1) newNodes.push(clone ? imageNode.cloneNode(true) : imageNode);
          });
        } else {
          newNodes.push(node);
        }
...
function formatAsShorthandTimePeriod(timeInMs, roundSeconds = false, showMs = false, padding = false) {
  const time = getMSAsTime(timeInMs);
  if (roundSeconds && time.milliseconds >= 500) time.seconds++;
  const timePeriods = [];
  if (time.years > 0) {
...
function getTemplateElement(templateID) {
  ...
  if (template === null || !(template instanceof HTMLTemplateElement)) throw new Error('Template does not exist');
  return template;
}
...
function getAnyElementFromFragment(fragment, elementID) {
  ...
  if (elem === null) throw new Error(`Fragment does not contain element with id: ${elementID}`);
  elem.removeAttribute('id');
  return elem;
}
function getElementFromFragment(fragment, elementID, tagName, preserveID = false) {
  ...
  if (!preserveID) elem.removeAttribute('id');
  return elem;
}
function formatFixed(num, digits) {
  try {
    if (num === 0) num = 0;
    return num.toLocaleString(setLang, { minimumFractionDigits: digits, maximumFractionDigits: digits });
  } catch (_a) {
...
class DataConstructionError extends Error {
  constructor(className, inner, id) {
    ...
    if (id !== undefined) message += ` with id "${id}`;
    message += ': ';
    if (inner !== undefined && inner instanceof Error) message += inner.message;
    super(message);
    this.inner = inner;
  }
  get name() {
...
class UnregisteredApplyDataModError extends Error {
  constructor(objectBeingModded, unregisteredName, unregisteredID) {
    super(
      `Error applying data modification to ${objectBeingModded.constructor.name} with id: ${objectBeingModded.id}. ${unregisteredName} with id: ${unregisteredID} is not registered.`,
    );
    this.name = this.constructor.name;
  }
}
...
class DropTable {
  ...
  unregisterDrops(data) {
    data.forEach((data) => {
      ...
      if (dropIndex < 0) {
        console.warn(
          `Warning when removing drop from loot table, Item with ID: ${data} does not exist in the loot table.`,
        );
        return;
      }
  ...
  getDrop() {
    ...
    return { item: drop.item, quantity };
  }
  getRawDrop() {
    const drop = selectFromWeightedArray(this.drops, this.totalWeight);
    const quantity = rollInteger(drop.minQuantity, drop.maxQuantity);
    return { drop: drop, quantity };
  }
  getAverageDropValue() {
...
class RandomModifierTable {
  ...
  getExcludedTable(existing) {
    ...
    const excludedDrops = this.drops.filter((drop) => {
      ...
      if (include) excludedWeight += drop.weight;
      return include;
    });
...
class SparseNumericMap {
  ...
  set(key, value) {
    if (value === 0) this.data.delete(key);
...
function printUnlockRequirements(requirements) {
  ...
  requirements.forEach((requirement) => {
    if (game.checkRequirement(requirement)) return;
    unlockElements.push(
      createUnlockElement(requirement.getNodes('skill-icon-xs mr-1 ml-1'), game.checkRequirement(requirement)),
    );
  });
...
function createCopyOfObject(obj) {
  ...
  if (type === 'object') return cloneObj(obj);
...
function reverseCombatTriangle(triangle) {
  ...
  const newTriangle = { damageModifier: {}, reductionModifier: {} };
...
function generateMonsterStatsCSV() {
  ...
  game.monsters.forEach((monster) => {
    ...
    a += `${
      (_c =
        (_b = monster.equipmentStats.find((stat) => stat.key === 'attackSpeed')) === null || _b === void 0
          ? void 0
    ...
    a += `${
      (_e =
        (_d = monster.equipmentStats.find((stat) => stat.key === 'stabAttackBonus')) === null || _d === void 0
          ? void 0
    ...
    a += `${
      (_g =
        (_f = monster.equipmentStats.find((stat) => stat.key === 'slashAttackBonus')) === null || _f === void 0
          ? void 0
    ...
    a += `${
      (_j =
        (_h = monster.equipmentStats.find((stat) => stat.key === 'blockAttackBonus')) === null || _h === void 0
          ? void 0
    ...
    a += `${
      (_l =
        (_k = monster.equipmentStats.find((stat) => stat.key === 'rangedAttackBonus')) === null || _k === void 0
          ? void 0
    ...
    a += `${
      (_o =
        (_m = monster.equipmentStats.find((stat) => stat.key === 'magicAttackBonus')) === null || _m === void 0
          ? void 0
    ...
    a += `${
      (_q =
        (_p = monster.equipmentStats.find((stat) => stat.key === 'meleeStrengthBonus')) === null || _p === void 0
          ? void 0
          : _p.value) !== null && _q !== void 0
        ? _q
        : ''
    }\t`;
    a += `${
      (_s =
        (_r = monster.equipmentStats.find((stat) => stat.key === 'rangedStrengthBonus')) === null || _r === void 0
          ? void 0
          : _r.value) !== null && _s !== void 0
        ? _s
        : ''
    }\t`;
    a += `${
      (_u =
        (_t = monster.equipmentStats.find((stat) => stat.key === 'magicDamageBonus')) === null || _t === void 0
          ? void 0
    ...
    a += `${
      (_w =
        (_v = monster.equipmentStats.find((stat) => stat.key === 'meleeDefenceBonus')) === null || _v === void 0
          ? void 0
          : _v.value) !== null && _w !== void 0
        ? _w
        : ''
    }\t`;
    a += `${
      (_y =
        (_x = monster.equipmentStats.find((stat) => stat.key === 'rangedDefenceBonus')) === null || _x === void 0
          ? void 0
          : _x.value) !== null && _y !== void 0
        ? _y
        : ''
    }\t`;
    a += `${
      (_0 =
        (_z = monster.equipmentStats.find((stat) => stat.key === 'magicDefenceBonus')) === null || _z === void 0
          ? void 0
    ...
    a += `${
      (_1 = monster.equipmentStats.find(
        (stat) => stat.key === 'resistance' && stat.damageType.id === 'melvorD:Normal',
      )) !== null && _1 !== void 0
        ? _1
        : ''
    }\t`;
    a += `${
      (_2 = monster.equipmentStats.find(
        (stat) => stat.key === 'resistance' && stat.damageType.id === 'melvorItA:Abyssal',
      )) !== null && _2 !== void 0
        ? _2
        : ''
    }\t`;
  ...
  game.items.forEach((item) => {
    ...
    if (item.id.includes('melvorItA')) {
      if (item instanceof EquipmentItem) {
        ...
        a += `${
          (_b =
            (_a = item.equipmentStats.find((stat) => stat.key === 'attackSpeed')) === null || _a === void 0
              ? void 0
        ...
        a += `${
          (_d =
            (_c = item.equipmentStats.find((stat) => stat.key === 'stabAttackBonus')) === null || _c === void 0
              ? void 0
        ...
        a += `${
          (_f =
            (_e = item.equipmentStats.find((stat) => stat.key === 'slashAttackBonus')) === null || _e === void 0
              ? void 0
        ...
        a += `${
          (_h =
            (_g = item.equipmentStats.find((stat) => stat.key === 'blockAttackBonus')) === null || _g === void 0
              ? void 0
        ...
        a += `${
          (_k =
            (_j = item.equipmentStats.find((stat) => stat.key === 'rangedAttackBonus')) === null || _j === void 0
              ? void 0
        ...
        a += `${
          (_m =
            (_l = item.equipmentStats.find((stat) => stat.key === 'magicAttackBonus')) === null || _l === void 0
              ? void 0
        ...
        a += `${
          (_p =
            (_o = item.equipmentStats.find((stat) => stat.key === 'meleeStrengthBonus')) === null || _o === void 0
              ? void 0
              : _o.value) !== null && _p !== void 0
            ? _p
            : ''
        }\t`;
        a += `${
          (_r =
            (_q = item.equipmentStats.find((stat) => stat.key === 'rangedStrengthBonus')) === null || _q === void 0
              ? void 0
        ...
        a += `${
          (_t =
            (_s = item.equipmentStats.find((stat) => stat.key === 'magicDamageBonus')) === null || _s === void 0
              ? void 0
              : _s.value) !== null && _t !== void 0
            ? _t
            : ''
        }\t`;
        a += `${
          (_v =
            (_u = item.equipmentStats.find((stat) => stat.key === 'meleeDefenceBonus')) === null || _u === void 0
              ? void 0
              : _u.value) !== null && _v !== void 0
            ? _v
            : ''
        }\t`;
        a += `${
          (_x =
            (_w = item.equipmentStats.find((stat) => stat.key === 'rangedDefenceBonus')) === null || _w === void 0
              ? void 0
        ...
        a += `${
          (_z =
            (_y = item.equipmentStats.find((stat) => stat.key === 'magicDefenceBonus')) === null || _y === void 0
              ? void 0
              : _y.value) !== null && _z !== void 0
            ? _z
            : ''
        }\t`;
        a += `${
          (_1 =
            (_0 = item.equipmentStats.find((stat) => stat.key === 'summoningMaxhit')) === null || _0 === void 0
              ? void 0
              : _0.value) !== null && _1 !== void 0
            ? _1
            : ''
        }`;
        a += `${
          (_2 = item.equipmentStats.find(
            (stat) => stat.key === 'resistance' && stat.damageType.id === 'melvorD:Normal',
          )) !== null && _2 !== void 0
            ? _2
            : ''
...
function generateLangModifierCustomDescriptionForItem(item) {
  ...
  LANGS.forEach((lang) => {
    if (lang === 'carrot' || lang === 'lemon') return;
    setLang = lang;
...
const MOBILE_UI_ELEMENT_SHIFTS = [
  { element: 'combat-corruption-settings', destination: 'combat-menu-0' },
  { element: 'combat-attack-styles', destination: 'combat-menu-0' },
  { element: 'combat-player-stats', destination: 'combat-menu-4' },
  { element: 'combat-slayer-task-menu', destination: 'combat-menu-5' },
];
const DESKTOP_UI_ELEMENT_SHIFTS = [
  { element: 'combat-player-stats', destination: 'desktop-combat-menus' },
  { element: 'combat-slayer-task-menu', destination: 'desktop-combat-menus' },
  { element: 'combat-corruption-settings', destination: 'desktop-combat-menus' },
  { element: 'combat-attack-styles', destination: 'desktop-combat-menus' },
];
