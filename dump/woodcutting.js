...
class RandomWoodcuttingProduct {
  constructor(data, game) {
    ...
    try {
      ...
      if (data.minChance !== undefined) this.minChance = data.minChance;
    } catch (e) {
...
class WoodcuttingTree extends SingleProductRecipe {
  ...
  applyDataModification(data, game) {
    ...
    try {
      if (data.baseInterval !== undefined) this.baseInterval = data.baseInterval;
      if (data.randomProducts !== undefined) {
...
class Woodcutting extends GatheringSkill {
  constructor(namespace, game) {
    ...
    this._media = 'assets/media/skills/woodcutting/woodcutting.png';
  ...
  modifyData(data) {
    ...
    (_a = data.trees) === null || _a === void 0
      ...
      : _a.forEach((modData) => {
          ...
          if (tree === undefined) throw new UnregisteredDataModError(WoodcuttingTree.name, modData.id);
  ...
  postDataRegistration() {
    ...
    this.game.crafting.actions.forEach((recipe) => {
      if (
        (recipe.category.id === 'melvorF:Necklaces' || recipe.category.id === 'melvorF:Rings') &&
  ...
  getUncappedDoublingChance(action) {
    ...
    if (this.game.modifiers.halveWoodcuttingDoubleChance > 0) chance = Math.floor(chance / 2);
    return chance;
  }
  ...
  selectTree(tree) {
    ...
    if (this.activeTrees.has(tree)) {
      ...
      this.activeTrees.size < this.treeCutLimit &&
  ...
  getRandomProductInfo(item) {
    ...
    quantity += this.game.modifiers.getValue('melvorD:flatBaseRandomProductQuantity', query);
  ...
  addArrowShaftReward(tree, rewards) {
    var _a;
    const shaftRecipe = this.game.fletching.actions.getObjectByID('melvorF:Arrow_Shafts');
  ...
  get actionRewards() {
    ...
    this.activeTrees.forEach((tree) => {
      ...
      const modifierProducts = this.game.modifiers.query(
        'melvorD:additionalItemBasedOnPrimaryQuantityChance',
        this.getActionItemModifierQuery(tree),
      );
      if (modifierProducts.length > 0) {
        ...
        productChances.forEach((chance, item) => {
          if (rollPercentage(chance)) rewards.addItem(item, logQuantity);
        });
      }
      ...
      if (rollForOffItem(chance)) {
        switch (item.id) {
          case 'melvorD:Bird_Nest':
            ...
            break;
          case 'melvorItA:Shadow_Drake_Nest':
...
Active Trees: ${treeLog.join('\n')}`;
  ...
  renderProgressBar() {
    ...
    if (this.isActive) woodcuttingMenu.progressBar.animateProgressFromTimer(this.actionTimer);
  ...
  decode(reader, version) {
    ...
    if (this.isActive && this.activeTrees.size === 0) this.shouldResetAction = true;
