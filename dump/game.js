...
class Game extends GameEventEmitter {
  constructor() {
    ...
    this.MIN_OFFLINE_TIME = 60000;
    this.MAX_OFFLINE_TIME = 86400000;
    this.OFFLINE_EXIT_TIME = 500;
    this.OFFLINE_GC_RATIO = 0.95;
    this._isInOnlineLoop = true;
    this._offlineInfo = { startTime: 0, timeProcessed: 0, tickRate: 1000 };
    ...
    this.registeredNamespaces.registerNamespace('melvorBaseGame', 'Base Game', false);
    this.registeredNamespaces.registerNamespace('melvorTrue', 'True', false);
    const demoNamespace = this.registeredNamespaces.registerNamespace('melvorD', 'Demo', false);
    if (cloudManager.hasFullVersionEntitlement) {
      this.registeredNamespaces.registerNamespace('melvorF', 'Full Version', false);
      this.registeredNamespaces.registerNamespace('melvorBirthday2023', 'Birthday Event 2023', false);
      this.registeredNamespaces.registerNamespace('melvorAprilFools2024', 'April Fools Event 2024', false);
    }
    if (cloudManager.hasTotHEntitlementAndIsEnabled)
      this.registeredNamespaces.registerNamespace('melvorTotH', 'Throne of the Herald', false);
    if (cloudManager.hasAoDEntitlementAndIsEnabled)
      this.registeredNamespaces.registerNamespace('melvorAoD', 'Atlas of Discovery', false);
    if (cloudManager.hasItAEntitlementAndIsEnabled)
      this.registeredNamespaces.registerNamespace('melvorItA', 'Into The Abyss', false);
    ...
    this.defaultRealm = new Realm(
      demoNamespace,
      { id: 'Melvor', name: 'Melvor Realm', media: 'assets/media/main/logo_no_text.png', unlockRequirements: [] },
      this,
    );
    this.realms.registerObject(this.defaultRealm);
    this.normalDamage = new DamageType(
      demoNamespace,
      {
        id: 'Normal',
        name: 'Normal Damage',
    ...
    this.emptyEquipmentItem = new EquipmentItem(
      demoNamespace,
      {
        ...
        category: '',
        type: '',
        media: 'assets/media/skills/combat/food_empty.png',
    ...
    this.emptyFoodItem = new FoodItem(
      demoNamespace,
      {
        itemType: 'Food',
        id: 'Empty_Food',
    ...
    if (cloudManager.hasAoDEntitlementAndIsEnabled) {
      const expac2Namespace = this.registeredNamespaces.getNamespace('melvorAoD');
      if (expac2Namespace === undefined) throw new Error('Atlas of Discovery Namespace not registered.');
    ...
    if (cloudManager.hasItAEntitlementAndIsEnabled) {
      const expac3Namespace = this.registeredNamespaces.getNamespace('melvorItA');
      if (expac3Namespace === undefined) throw new Error('Into the Abyss Namespace not registered.');
  ...
  get playerAbyssalCombatLevel() {
    ...
    const melee =
      0.325 * (this.attack.level + this.attack.abyssalLevel + this.strength.level + this.strength.abyssalLevel);
  ...
  fetchAndRegisterDataPackage(url) {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      if (!response.ok)
        throw new Error(
          `Could not fetch data package with URL: ${url}. Error ${response.status}: ${response.statusText}`,
        );
  ...
  registerDataPackage(dataPackage) {
    var _a;
    if (dataPackage.namespace === undefined) throw new Error(`Package does not have a namespace defined.`);
    const namespace = this.registeredNamespaces.getNamespace(dataPackage.namespace);
    if (namespace === undefined)
      throw new Error(
        `Error trying to register data package. Namespace: "${dataPackage.namespace}" is not registered.`,
      );
    if (dataPackage.data !== undefined) this.registerGameData(namespace, dataPackage.data);
    (_a = dataPackage.dependentData) === null || _a === void 0
      ? void 0
      : _a.forEach((depData) => {
          if (this.registeredNamespaces.hasNamespace(depData.namespace)) {
            if (depData.data !== undefined) this.registerGameData(namespace, depData.data);
            if (depData.modifications !== undefined) this.applyDataModifications(depData.modifications);
          }
        });
    if (dataPackage.modifications !== undefined) this.applyDataModifications(dataPackage.modifications);
  ...
  registerGameData(namespace, gameData) {
    if (gameData.realms !== undefined) this.registerRealms(namespace, gameData.realms);
    if (gameData.damageTypes !== undefined) this.registerDamageTypes(namespace, gameData.damageTypes);
    if (gameData.modifiers !== undefined) this.registerModifiers(namespace, gameData.modifiers);
    ...
    if (gameData.combatEffects !== undefined) this.registerCombatEffects(namespace, gameData.combatEffects);
    if (gameData.combatEffectTables !== undefined)
      this.registerCombatEffectTables(namespace, gameData.combatEffectTables);
    if (gameData.combatPassives !== undefined) this.registerCombatPassiveData(namespace, gameData.combatPassives);
    if (gameData.attacks !== undefined) this.registerAttackData(namespace, gameData.attacks);
    if (gameData.pages !== undefined) this.registerPages(namespace, gameData.pages);
    if (gameData.pets !== undefined) this.registerPets(namespace, gameData.pets);
    if (gameData.attackStyles !== undefined) this.registerAttackStyles(namespace, gameData.attackStyles);
    if (gameData.prayers !== undefined) this.registerPrayerData(namespace, gameData.prayers);
    if (gameData.equipmentSlots !== undefined) this.registerEquipmentSlotData(namespace, gameData.equipmentSlots);
    if (gameData.items !== undefined) this.registerItemData(namespace, gameData.items);
    if (gameData.bankSortOrder !== undefined) this.bank.registerSortOrder(gameData.bankSortOrder);
    if (gameData.itemUpgrades !== undefined) this.bank.registerItemUpgrades(gameData.itemUpgrades);
    if (gameData.itemSynergies !== undefined) this.registerItemSynergies(gameData.itemSynergies);
    if (gameData.randomGems !== undefined) this.randomGemTable.registerDrops(this, gameData.randomGems);
    ...
    if (gameData.randomFragments !== undefined) this.randomFragmentTable.registerDrops(this, gameData.randomFragments);
    if (gameData.randomFiremakingOils !== undefined)
      this.randomFiremakingOilTable.registerDrops(this, gameData.randomFiremakingOils);
    if (gameData.golbinRaid !== undefined) this.golbinRaid.registerData(gameData.golbinRaid);
    ...
    if (gameData.gamemodes !== undefined) this.registerGamemodes(namespace, gameData.gamemodes);
    if (gameData.attackSpellbooks !== undefined) this.registerAttackSpellbookData(namespace, gameData.attackSpellbooks);
    if (gameData.attackSpells !== undefined) this.registerAttackSpellData(namespace, gameData.attackSpells);
    if (gameData.standardSpells !== undefined) this.registerStandardSpellData(namespace, gameData.standardSpells);
    if (gameData.ancientSpells !== undefined) this.registerAncientSpellData(namespace, gameData.ancientSpells);
    if (gameData.archaicSpells !== undefined) this.registerArchaicSpellData(namespace, gameData.archaicSpells);
    if (gameData.auroraSpells !== undefined) this.registerAuroraSpellData(namespace, gameData.auroraSpells);
    if (gameData.curseSpells !== undefined) this.registerCurseSpellData(namespace, gameData.curseSpells);
    if (gameData.monsters !== undefined) this.registerMonsterData(namespace, gameData.monsters);
    if (gameData.itmMonsters !== undefined) this.registerRandomMonsters(gameData.itmMonsters, this.combat.itmMonsters);
    ...
    if (gameData.combatAreas !== undefined) this.registerCombatAreaData(namespace, gameData.combatAreas);
    if (gameData.slayerAreas !== undefined) this.registerSlayerAreaData(namespace, gameData.slayerAreas);
    if (gameData.combatEvents !== undefined) this.registerCombatEventData(namespace, gameData.combatEvents);
    if (gameData.dungeons !== undefined) this.registerDungeonData(namespace, gameData.dungeons);
    if (gameData.abyssDepths !== undefined) this.registerAbyssDepthData(namespace, gameData.abyssDepths);
    if (gameData.strongholds !== undefined) this.registerStrongholdData(namespace, gameData.strongholds);
    ...
    if (gameData.shopCategories !== undefined) this.registerShopCategories(namespace, gameData.shopCategories);
    if (gameData.shopCategoryOrder !== undefined)
      this.shop.categoryDisplayOrder.registerData(gameData.shopCategoryOrder);
    if (gameData.shopPurchases !== undefined) this.registerShopPurchases(namespace, gameData.shopPurchases);
    if (gameData.shopDisplayOrder !== undefined) this.shop.purchaseDisplayOrder.registerData(gameData.shopDisplayOrder);
    if (gameData.shopUpgradeChains !== undefined) this.registerShopUpgradeChains(namespace, gameData.shopUpgradeChains);
    if (gameData.skillTreesDisplayOrder !== undefined)
      this.skillTreesDisplayOrder.registerData(gameData.skillTreesDisplayOrder);
    if (gameData.ancientRelics !== undefined) this.registerAncientRelics(namespace, gameData.ancientRelics);
    ...
    if (gameData.skillData !== undefined) {
      gameData.skillData.forEach((skillsData) => {
        ...
        if (skill === undefined)
          throw new Error(
            `Error registering data package. Cannot register data for unregistered skill: ${skillsData.skillID}.`,
          );
    ...
    if (gameData.lore !== undefined) this.lore.registerLore(namespace, gameData.lore);
    if (!namespace.isModded) {
      if (gameData.tutorialStages !== undefined) this.tutorial.registerStages(namespace, gameData.tutorialStages);
      if (gameData.tutorialStageOrder !== undefined) this.tutorial.registerStageOrder(gameData.tutorialStageOrder);
    }
  ...
  queueForSoftDependencyReg(data, object, where) {
    this.softDataRegQueue.push({ data, object, where });
  }
  postDataRegistration() {
    ...
    this.combatAreas.forEach((area) => {
      if (area.allowAutoJump) area.monsters.forEach((monster) => this.monsterAreas.set(monster, area));
    });
  ...
  registerItemData(namespace, data) {
    data.forEach((itemData) => {
      if (itemData.isDebug && !DEBUGENABLED) return;
    ...
    data.forEach((data) => {
      ...
      if ('templateID' in data) {
        ...
        if (template === undefined)
          throw new Error(
            `Error registering CombatEffect with id: ${data.id}. CombatEffectTemplate with id: ${data.templateID} is not registered.`,
          );
  ...
  registerCombatPassiveData(namespace, data) {
    if (namespace.name === 'melvorF') this.combatPassives.registerObject(new ControlledAffliction(namespace, this));
  ...
  registerSkillLevelCapIncreases(namespace, data) {
    data.forEach((capData) =>
      this.skillLevelCapIncreases.registerObject(new SkillLevelCapIncrease(namespace, capData, this)),
    );
  }
  ...
  registerSkill(namespace, constructor) {
    ...
    if (isAction) this.actions.registerObject(skillInstance);
  ...
  applyDataModifications(modificationData) {
    ...
    if (modificationData.modifiers !== undefined) {
      modificationData.modifiers.forEach((modData) => {
        ...
        if (modifier === undefined) throw new UnregisteredDataModError(Modifier.name, modData.id);
    ...
    if (modificationData.gamemodes !== undefined) {
      modificationData.gamemodes.forEach((modData) => {
        ...
        if (gamemode === undefined) throw new UnregisteredDataModError(Gamemode.name, modData.id);
    ...
    if (modificationData.combatAreas !== undefined) {
      modificationData.combatAreas.forEach((modData) => {
        ...
        if (combatArea === undefined) throw new UnregisteredDataModError(CombatArea.name, modData.id);
    ...
    if (modificationData.dungeons !== undefined) {
      modificationData.dungeons.forEach((modData) => {
        ...
        if (dungeon === undefined) throw new UnregisteredDataModError(Dungeon.name, modData.id);
    ...
    if (modificationData.items !== undefined) {
      modificationData.items.forEach((modData) => {
        ...
        if (item === undefined) throw new UnregisteredDataModError(Item.name, modData.id);
    ...
    if (modificationData.monsters !== undefined) {
      modificationData.monsters.forEach((modData) => {
        ...
        if (monster === undefined) throw new UnregisteredDataModError(Monster.name, modData.id);
    ...
    if (modificationData.shopPurchases !== undefined) {
      modificationData.shopPurchases.forEach((modData) => {
        ...
        if (purchase === undefined) throw new UnregisteredDataModError(ShopPurchase.name, modData.id);
    ...
    if (modificationData.slayerAreas !== undefined) {
      modificationData.slayerAreas.forEach((modData) => {
        ...
        if (slayerAreas === undefined) throw new UnregisteredDataModError(SlayerArea.name, modData.id);
        slayerAreas.applyDataModification(modData, this);
      });
    }
    (_a = modificationData.shopUpgradeChains) === null || _a === void 0
      ? void 0
      : _a.forEach((modData) => {
          const upgradeChain = this.shop.upgradeChains.getObjectByID(modData.id);
          if (upgradeChain === undefined) throw new UnregisteredDataModError(ShopUpgradeChain.name, modData.id);
          upgradeChain.applyDataModification(modData, this);
        });
    (_b = modificationData.cookingCategories) === null || _b === void 0
      ? void 0
      : _b.forEach((modData) => {
          const category = this.cooking.categories.getObjectByID(modData.id);
          if (category === undefined) throw new UnregisteredDataModError(CookingCategory.name, modData.id);
          category.applyDataModification(modData, this);
        });
    (_c = modificationData.fletchingRecipes) === null || _c === void 0
      ? void 0
      : _c.forEach((modData) => {
          const recipe = this.fletching.actions.getObjectByID(modData.id);
          if (recipe === undefined) throw new UnregisteredDataModError(FletchingRecipe.name, modData.id);
    ...
    if (modificationData.pages !== undefined) {
      modificationData.pages.forEach((modData) => {
        ...
        if (page === undefined) throw new UnregisteredDataModError(Page.name, modData.id);
    ...
    if (modificationData.equipmentSlots !== undefined) {
      modificationData.equipmentSlots.forEach((modData) => {
        ...
        if (equipmentSlot === undefined) throw new UnregisteredDataModError(EquipmentSlot.name, modData.id);
    ...
    if (modificationData.damageTypes !== undefined) {
      modificationData.damageTypes.forEach((modData) => {
        ...
        if (damageType === undefined) throw new UnregisteredDataModError(EquipmentSlot.name, modData.id);
    ...
    if (modificationData.combatAreaCategories !== undefined) {
      modificationData.combatAreaCategories.forEach((modData) => {
        ...
        if (category === undefined) throw new UnregisteredDataModError(CombatAreaCategory.name, modData.id);
        category.applyDataModification(modData, this);
      });
    }
    if (modificationData.skillData !== undefined) {
      modificationData.skillData.forEach(({ skillID, data }) => {
        ...
        if (skill === undefined) throw new UnregisteredDataModError(Skill.name, skillID);
    ...
    if (modificationData.skillLevelCapIncreases !== undefined) {
      modificationData.skillLevelCapIncreases.forEach((modData) => {
        ...
        if (capIncrease === undefined) throw new UnregisteredDataModError(SkillLevelCapIncrease.name, modData.id);
    ...
    if (modificationData.pets !== undefined) {
      modificationData.pets.forEach((modData) => {
        ...
        if (pet === undefined) throw new UnregisteredDataModError(Pet.name, modData.id);
    ...
    return statsData.map((data) => {
      switch (data.key) {
        case 'damageReduction':
          return { key: 'resistance', value: data.value, damageType: this.normalDamage };
        case 'resistance':
        case 'summoningMaxhit':
          return {
            ...
            value: data.value,
            damageType:
              data.damageType === undefined ? this.normalDamage : this.damageTypes.getObjectSafe(data.damageType),
          };
        default:
  ...
  modifyEquipStats(equipStats, modData) {
    ...
    if (modData.add !== undefined) {
      ...
      addStats.forEach((stat) => {
        const existing = equipStats.find(
          (e) =>
            e.key === stat.key &&
            ((e.key !== 'resistance' && e.key !== 'summoningMaxhit') || e.damageType === stat.damageType),
        );
        if (existing !== undefined) existing.value += stat.value;
        else equipStats.push(stat);
      });
    }
    return equipStats;
  }
    ...
    Object.entries(data).forEach(([key, valuesData]) => {
      ...
      if (alias !== undefined) {
        valuesData.forEach((valueData) => {
          if (alias.isNegative) valueData.value = -valueData.value;
  ...
  _modifyModifierValues(modifiers, modData, getAlias, getValues) {
    if (modData.remove !== undefined) {
      const removeIds = modData.remove.map((key) => {
        ...
        if (alias !== undefined) return alias.modifier.id;
  ...
  modifyModifierValues(modifiers, modData) {
    return this._modifyModifierValues(
      modifiers,
      modData,
      (key) => this.modifierRegistry.getAlias(key),
      (data) => this.getModifierValuesFromData(data),
    );
  }
  modifyEnemyModifierValues(modifiers, modData) {
    return this._modifyModifierValues(
      modifiers,
      modData,
      (key) => this.modifierRegistry.getEnemyAlias(key),
      (data) => this.getEnemyModifierValuesFromData(data),
    );
  }
  getCombatEffectApplicatorFromData(data) {
    if ('tableID' in data) return TableCombatEffectApplicator.fromData(data, this);
    else return SingleCombatEffectApplicator.fromData(data, this);
  }
  ...
  getCombatEffectApplicatorWithTriggerFromData(data) {
    if ('tableID' in data) return TableCombatEffectApplicator.fromDataWithTrigger(data, this);
    else return SingleCombatEffectApplicator.fromDataWithTrigger(data, this);
  }
  ...
  modifyCombatEffectApplicators(applicators, modData, className = 'Unknown') {
    if (modData.removeEffect !== undefined) {
      modData.removeEffect.forEach((effectID) => {
        ...
        if (effectIndex < 0) {
          console.warn(
            `Warning when removing combat effect applicator from ${className}, effect applicator with effect: ${effectID} does not exist in the ${className}'s combat effect applicators.`,
          );
          return;
        }
    ...
    if (modData.removeTable !== undefined) {
      modData.removeTable.forEach((tableID) => {
        ...
        if (effectIndex < 0) {
          console.warn(
            `Warning when removing table combat effect applicator from ${className}, effect applicator with effect table: ${tableID} does not exist in the ${className}'s combat effect applicators.`,
          );
          return;
        }
        applicators.splice(effectIndex, 1);
  ...
  getEquipmentSlotID(slotID) {
    if (!slotID.includes(':')) slotID = `${'melvorD'}:${slotID}`;
    return slotID;
  }
  ...
  getSkillValuesFromData(data) {
    return data.map(({ skillID, value }) => {
      return { skill: this.skills.getObjectSafe(skillID), value };
    });
  }
  getDummyData(fullID) {
    ...
    if (dataNamespace === undefined) {
      dataNamespace = this.dummyNamespaces.registerNamespace(
        name,
        'Error: Unregistered Namespace',
        !name.startsWith('melvor'),
      );
    }
  ...
  startMainLoop() {
    if (!this.loopStarted) {
      ...
      if (DEBUGENABLED) console.warn('Game loop was already started.');
    }
  }
  stopMainLoop() {
    if (this.loopStarted) {
      ...
      if (DEBUGENABLED) console.warn('Game loop was already stopped');
    }
  }
  blockInteraction() {
    var _a;
    (_a = document.getElementById('interaction-blocker')) === null || _a === void 0
      ? void 0
  ...
  idleChecker(action) {
    if (this.isGolbinRaid) return true;
    if (this.activeAction === undefined) return false;
  ...
  onLoad() {
    ...
    if (this.golbinRaid.raidRunning && this.activeAction !== this.golbinRaid) {
      if (this.pausedAction !== undefined) {
        ...
        if (!(paused instanceof BaseManager) && !paused.isActive) this.pausedAction = undefined;
      }
    ...
    if (this.isPaused && this.pausedAction === undefined && !this.isGolbinRaid) this._isPaused = false;
    ...
    if (this.activeAction !== undefined && !this.activeAction.isActive) this.activeAction = undefined;
  ...
  setUpGamemodeOnLoad() {
    ...
    (_a = this.currentGamemode.initialLevelCaps) === null || _a === void 0
      ? void 0
      : _a.forEach(({ skill, value }) => {
          if (!skill.levelCapSet) skill.setLevelCap(value);
        });
    const defaultLevelCap = this.currentGamemode.defaultInitialLevelCap;
    if (defaultLevelCap !== undefined) {
      this.skills.forEach((skill) => {
        if (!skill.levelCapSet) skill.setLevelCap(defaultLevelCap);
      });
    }
    (_b = this.currentGamemode.initialAbyssalLevelCaps) === null || _b === void 0
      ? void 0
      : _b.forEach(({ skill, value }) => {
          if (!skill.abyssalLevelCapSet) skill.setAbyssalLevelCap(value);
        });
    const defaultAbyssalLevelCap = this.currentGamemode.defaultInitialAbyssalLevelCap;
    if (defaultAbyssalLevelCap !== undefined) {
      this.skills.forEach((skill) => {
        if (!skill.abyssalLevelCapSet) skill.setAbyssalLevelCap(defaultAbyssalLevelCap);
      });
    }
    this.currentGamemode.levelCapIncreases.forEach((capIncrease) => {
      if (this.activeLevelCapIncreases.includes(capIncrease)) return;
      ...
      if (checkSetIncreases) {
        capIncrease.setIncreases.forEach(({ skill, value }) => {
          switch (capIncrease.levelType) {
            case 'Standard':
              if (skill.isLevelCapBelow(value)) skill.applySetLevelCap(value);
              break;
            case 'Abyssal':
              if (skill.isAbyssalLevelCapBelow(value)) skill.applySetAbyssalLevelCap(value);
              break;
          }
        });
      }
    ...
    if (this.currentGamemode.abyssalLevelCapCost !== undefined) {
      this.currentGamemode.abyssalLevelCapCost.skillLevelGates.forEach(
        (skill) => (skill.isGatingAbyssalLevelCapPurchases = true),
      );
    }
  ...
  queueRequirementRenders() {
    ...
    if (((_a = game.openPage) === null || _a === void 0 ? void 0 : _a.id) === 'melvorD:Township') {
  ...
  checkUniqueBirthdayEventCompletions() {
    if (!cloudManager.isBirthdayEvent2023Active()) return;
    if (!this.birthdayEvent2023CompletionTracker[2]) {
      const present1 = this.items.getObjectByID('melvorF:Birthday_Present_Gathering');
      const present2 = this.items.getObjectByID('melvorF:Birthday_Present_Artisan');
      const present3 = this.items.getObjectByID('melvorF:Birthday_Present_Utility');
      const present4 = this.items.getObjectByID('melvorF:Birthday_Present_Combat');
      if (present1 === undefined || present2 === undefined || present3 === undefined || present4 === undefined) return;
      const presentsFound = [present1, present2, present3, present4].every(
        (present) => this.stats.itemFindCount(present) > 0,
      );
      this.birthdayEvent2023CompletionTracker[2] = presentsFound;
    }
    if (!this.birthdayEvent2023CompletionTracker[3]) {
      this.birthdayEvent2023CompletionTracker[3] = true;
    }
  }
  updateBirthdayEventProgress() {
    ...
    if (this.birthdayEvent2023CompletionTracker.every((isComplete) => isComplete)) {
      localStorage.setItem('birthdayGamemodeUnlocked', 'true');
    }
  }
  renderBirthdayEventProgress() {
    if (!this.renderQueue.birthdayEventProgress || !cloudManager.isBirthdayEvent2023Active()) return;
  ...
  renderCombatMinibar() {
    var _a;
    if (!this.renderQueue.combatMinibar) return;
    ...
    if (
      ...
      !(
        ...
        !this.settings.showCombatMinibarCombat
      )
    ) {
      showElement(minibar);
    } else {
  ...
  renderSidebarSkillUnlock() {
    if (!this.renderQueue.sidebarSkillUnlock) return;
    const sidebarItem = sidebar.category('').item('Ancient Relic Skill Unlock');
    if (this.levelCapIncreasesBeingSelected.length === 0) {
      if (sidebarItem.rootEl) sidebarItem.rootEl.classList.add('d-none');
    } else {
      if (sidebarItem.rootEl) sidebarItem.rootEl.classList.remove('d-none');
      const totalLeft = this.levelCapIncreasesBeingSelected.reduce(
        (prev, increase) => prev + increase.randomIncreasesLeft,
        0,
      );
      if (sidebarItem.asideEl) sidebarItem.asideEl.textContent = `${totalLeft}`;
    }
    this.renderQueue.sidebarSkillUnlock = false;
  }
  renderActiveSkills() {
    if (!this.renderQueue.activeSkills) return;
  ...
  triggerOfflineLoop() {
    if (!this._isInOnlineLoop || this.isGolbinRaid) return;
  ...
  loop() {
    ...
    if (this.interactionBlocked) this.unblockInteraction();
  }
  enterOfflineLoop(loopTime) {
    ...
    (_b = (_a = this.activeAction) === null || _a === void 0 ? void 0 : _a.createOfflineSnapshot) === null ||
    _b === void 0
      ? void 0
      : _b.call(_a);
    pauseModalQueue();
  ...
  loopOnline(loopTime) {
    if (!this.isGolbinRaid) this.tickTimestamp = loopTime;
    try {
      this.processTime();
    } catch (e) {
      this.stopMainLoop();
...
${stackTrace}`;
    ...
    equipmentToLog.equippedArray.forEach((equippedItem) => {
      if (equippedItem.providesStats) equipmentText += `${equippedItem.slot.id}: ${equippedItem.item.id}\n`;
    });
...
${moddingLog}`;
  ...
  clearActionIfActiveOrPaused(action) {
    if (this.pausedAction === action) this.pausedAction = undefined;
  ...
  trackOfflineTelemetry(oldSnapshot, newSnapshot, timeDiff) {
    ...
    this.items.forEach((item) => {
      ...
      const qtyDiff =
        ((_a = newSnapshot.bank.get(item)) !== null && _a !== void 0 ? _a : 0) -
        ((_b = oldSnapshot.bank.get(item)) !== null && _b !== void 0 ? _b : 0);
      if (qtyDiff > 0) {
        this.telemetry.createItemGainedEvent(
          item,
          qtyDiff,
          `Skill.${
            (_d = (_c = this.activeAction) === null || _c === void 0 ? void 0 : _c.id) !== null && _d !== void 0
              ? _d
              : 'Unknown'
          }`,
        );
      } else if (qtyDiff < 0) {
        this.telemetry.createItemRemovedFromBankEvent(
          item,
          qtyDiff,
          `Skill.${
            (_f = (_e = this.activeAction) === null || _e === void 0 ? void 0 : _e.id) !== null && _f !== void 0
              ? _f
              : 'Unknown'
          }`,
        );
    ...
    this.telemetry.fireEventType('offline_xp_gain');
    this.telemetry.fireEventType('offline_abyssal_xp_gain');
  ...
  updateRichPresence(time) {
    if (time - this._lastRichPresenceUpdate >= RICH_PRESENCE_UPDATE_INTERVAL) {
      ...
      if (!connectedToSteam || cloudManager.isTest) return;
  ...
  gameInteractionUpdate(time) {
    ...
    if (
      !this._frameRateThrottled &&
      ((this.settings.throttleFrameRateOnInactivity && this._inactivityTime <= 0) ||
        ((_a = this.openPage) === null || _a === void 0 ? void 0 : _a.id) !== 'melvorAoD:Cartography')
    ) {
  ...
  getHeaderFromSaveString(saveString) {
    ...
    return __awaiter(this, void 0, void 0, function* () {
      try {
        ...
        if (header.saveVersion > currentSaveVersion) return 2;
        else return header;
      } catch (e) {
        if (e instanceof Error && e.message === 'String is not save.') {
          try {
            ...
            if (gamemodeID === undefined) throw new Error(`No Gamemode maps to: ${saveGame.currentGamemode}`);
  ...
  getSaveHeader() {
    ...
    if (offlineAction === undefined) offlineAction = this.pausedAction;
    const activeNamespaces = [];
    this.registeredNamespaces.forEach((namespace) => {
      if (!namespace.isModded) activeNamespaces.push(namespace.name);
    });
    return {
  ...
  encode(writer) {
    ...
    if (this.activeAction !== undefined) writer.writeNamespacedObject(this.activeAction);
    writer.writeBoolean(this.pausedAction !== undefined);
    if (this.pausedAction !== undefined) writer.writeNamespacedObject(this.pausedAction);
  ...
  decode(reader, version) {
    ...
    if (reader.getBoolean()) {
      ...
      if (typeof activeAction !== 'string') this.activeAction = activeAction;
    }
    if (reader.getBoolean()) {
    ...
    if (typeof gamemode === 'string') throw new Error('Error loading save. Gamemode is not registered.');
    ...
    if (version >= 51) this.keyboard.decode(reader, version);
    if (resetPaused) {
    ...
    if (version < 113) {
      ...
      if (skillUnlocksRemaining > 0) {
        if (this.currentGamemode.pre99RollConversion !== undefined) {
          ...
          if (skillUnlocks.length > 0) this.currentGamemode.pre99RollConversion.setSelectionFromSkills(skillUnlocks);
        }
      }
      if (skillUnlocksRemainingPost99 > 0) {
        if (this.currentGamemode.post99RollConversion !== undefined) {
          ...
          if (skillUnlocks.length > 0) this.currentGamemode.post99RollConversion.setSelectionFromSkills(skillUnlocks);
        }
      }
    }
    if (version >= 71) {
    ...
    if (version >= 100) {
      reader.getArray((reader) => {
        ...
        if (typeof currency === 'string') Currency.dumpData(reader, version);
        else currency.decode(reader, version);
      });
      reader.getArray((reader) => {
        const depth = reader.getNamespacedObject(this.abyssDepths);
        const timesCompleted = reader.getUint32();
        if (typeof depth !== 'string') depth.timesCompleted = timesCompleted;
      });
      reader.getArray((reader) => {
        const stronghold = reader.getNamespacedObject(this.strongholds);
        const timesCompleted = reader.getUint32();
        if (typeof stronghold !== 'string') stronghold.timesCompleted = timesCompleted;
      });
    }
    if (version >= 113) {
    ...
    if (version < 100 && (this.activeAction === this.thieving || this.activeAction === this.combat)) {
  ...
  deserialize(reader, version, idMap) {
    ...
    if (oldPausedAction !== undefined) {
      ...
      if (action !== undefined) this.pausedAction = action;
    }
  ...
  increaseSkillLevelCaps(capIncrease, reqSet) {
    if (!this.checkRequirements(reqSet.requirements)) return;
    switch (capIncrease.levelType) {
      case 'Standard':
    ...
    if (capIncrease.randomIncreases.length > 0 && capIncrease.randomCount > 0) {
      if (capIncrease.randomIncreasesLeft === 0) this.levelCapIncreasesBeingSelected.push(capIncrease);
  ...
  validateRandomLevelCapIncreases() {
    if (this.levelCapIncreasesBeingSelected.length === 0) return;
  ...
  queueNextRandomLevelCapModal(noModal = false) {
    if (this.levelCapIncreasesBeingSelected.length === 0) {
      if (levelCapIncreaseModal.isVisible) Swal.close();
      return;
    }
    if (levelCapIncreaseModal.isActive) return;
    const capIncrease = this.levelCapIncreasesBeingSelected[0];
    if (capIncrease.randomSelection.size === 0) capIncrease.rollRandomSelection();
    if (capIncrease.randomSelection.size === 0) {
      capIncrease.randomIncreasesLeft = 0;
      this.levelCapIncreasesBeingSelected = this.levelCapIncreasesBeingSelected.filter((s) => s !== capIncrease);
    ...
    if (!levelCapIncreaseModal.isVisible) {
      addModalToQueue({
        title:
          capIncrease.levelType === 'Standard'
  ...
  fireLevelCapIncreaseModal(skill) {
    if (this.currentGamemode.levelCapCost === undefined) return;
  ...
  fireAbyssalLevelCapIncreaseModal(skill) {
    if (this.currentGamemode.abyssalLevelCapCost === undefined) return;
    const content = createElement('level-cap-purchase-modal');
    customElements.upgrade(content);
  ...
  purchaseSkillLevelCaps(skill, amount = 1) {
    ...
    if (capCost === undefined || capCost.getMaxLevelIncreases(skill) < amount) return;
  ...
  purchaseAbyssalSkillLevelCaps(skill, amount = 1) {
    ...
    if (capCost === undefined || capCost.getMaxAbyssalLevelIncreases(skill) < amount) return;
  ...
  isItemOwned(item) {
    return (
      this.bank.hasItem(item) || (item instanceof EquipmentItem && this.combat.player.checkEquipmentSetsForItem(item))
    );
  }
  getMonsterArea(monster) {
    let area = this.monsterAreas.get(monster);
    if (area === undefined) area = this.unknownCombatArea;
    return area;
  }
  ...
  getPageForActiveAction() {
    if (this.activeAction !== undefined) {
      ...
      if (pages !== undefined) return pages[0];
    }
    const page = this.pages.getObjectByID('melvorD:Bank');
    if (page === undefined) throw new Error(`Error, Bank page not registered.`);
    return page;
  }
  ...
  isAchievementMet(achievement) {
    if (achievement.requiredGamemode !== undefined && this.currentGamemode !== achievement.requiredGamemode)
      return false;
    if (achievement.requirements.length > 0) return this.checkRequirements(achievement.requirements, false);
    switch (achievement.id) {
      ...
      case 'NEW_ACHIEVEMENT_3_20':
        return this.completion.itemProgress.getPercent('melvorBaseGame') === 100;
      case 'NEW_ACHIEVEMENT_1_27':
      case 'NEW_ACHIEVEMENT_3_21':
        return this.completion.petProgress.getPercent('melvorBaseGame') === 100;
      case 'NEW_ACHIEVEMENT_1_28':
      case 'NEW_ACHIEVEMENT_3_22':
        return this.completion.masteryProgress.getPercent('melvorBaseGame') === 100;
      case 'NEW_ACHIEVEMENT_1_29':
      case 'NEW_ACHIEVEMENT_3_23':
        return this.completion.monsterProgress.getPercent('melvorBaseGame') === 100;
      ...
      case 'NEW_ACHIEVEMENT_2_27': {
        const plant = this.monsters.getObjectByID('melvorD:Plant');
  ...
  setupCurrentGamemode() {
    ...
    if (startingSkills !== undefined) {
      ...
      this.skills.forEach((skill) => {
        skill.setUnlock(
          (cloudManager.hasFullVersionEntitlement || isDemoSkill(skill)) && skill.unlockRequirements.length === 0,
        );
      });
    }
    (_a = this.currentGamemode.initialLevelCaps) === null || _a === void 0
    ...
    if (this.currentGamemode.defaultInitialLevelCap !== undefined) {
      ...
      this.skills.forEach((skill) => {
        if (!skill.levelCapSet) skill.setLevelCap(cap);
      });
    }
    (_b = this.currentGamemode.initialAbyssalLevelCaps) === null || _b === void 0
      ? void 0
    ...
    if (this.currentGamemode.defaultInitialAbyssalLevelCap !== undefined) {
      ...
      this.skills.forEach((skill) => {
        if (!skill.abyssalLevelCapSet) skill.setAbyssalLevelCap(cap);
      });
    }
  ...
  computeTokenItemStats(updatePlayer = false) {
    ...
    if (updatePlayer) this.combat.computeAllStats();
  }
  ...
  convertFromOldFormat(save, idMap) {
    if (save.serialData !== undefined) {
      if (save.version <= 3) {
        ...
        if (save.offline.skill !== null && save.offline.skill === 9) this.activeAction = this.combat;
      } else this.deserialize(save.serialData, save.version, idMap);
    } else {
    ...
    if (save.version <= 15) {
      if (save.summoningData !== undefined) this.summoning.convertFromOldFormat(save.summoningData, idMap);
    ...
    if (save.glovesTracker !== undefined) this.itemCharges.convertFromOldFormat(save.glovesTracker, idMap);
    ...
    if (save.dungeonCompleteCount !== undefined) this.combat.convertDungeonCompletion(save.dungeonCompleteCount, idMap);
    ...
    if (save.offline !== undefined) this.convertOldOffline(save.offline, idMap);
    if (save.raidCoins !== undefined) this.raidCoins.set(save.raidCoins);
    if (save.gp !== undefined) this.gp.set(save.gp);
    this.tutorial.convertFromOldFormat(save, idMap);
    this.farming.convertFromOldFormat(save, idMap);
    if (save.username !== undefined) this.characterName = save.username;
    if (save.titleNewsID !== undefined) this.readNewsIDs = save.titleNewsID;
    if (save.gameUpdateNotification !== undefined) this.lastLoadedGameVersion = save.gameUpdateNotification;
  ...
  convertOldOffline(offline, idMap) {
    ...
    if (!this.mining.isActive && offline.skill === 4 && offline.action !== null) {
      this.mining.setFromOldOffline(offline, idMap);
    }
    if (!this.firemaking.isActive && offline.skill === 2 && offline.action !== null) {
      this.firemaking.setFromOldOffline(offline, idMap);
    }
    if (!this.woodcutting.isActive && offline.skill === 0 && offline.action !== null) {
      this.woodcutting.setFromOldOffline(offline, idMap);
    }
    if (!this.herblore.isActive && offline.skill === 19 && offline.action !== null) {
      this.herblore.setFromOldOffline(offline, idMap);
    }
    if (!this.smithing.isActive && offline.skill === 5 && offline.action !== null) {
      this.smithing.setFromOldOffline(offline, idMap);
    }
    if (!this.runecrafting.isActive && offline.skill === 15 && offline.action !== null) {
      this.runecrafting.setFromOldOffline(offline, idMap);
    }
    if (!this.altMagic.isActive && offline.skill === 16 && offline.action !== null) {
      this.altMagic.setFromOldOffline(offline, idMap);
    }
    if (!this.crafting.isActive && offline.skill === 14 && offline.action !== null) {
      this.crafting.setFromOldOffline(offline, idMap);
    }
    if (!this.fletching.isActive && offline.skill === 13 && offline.action !== null) {
      this.fletching.setFromOldOffline(offline, idMap);
    }
    if (!this.summoning.isActive && offline.skill === 21 && offline.action !== null) {
      this.smithing.setFromOldOffline(offline, idMap);
    }
    if (!this.fishing.isActive && offline.skill === 1 && offline.action !== null && Array.isArray(offline.action)) {
      this.fishing.setFromOldOffline(offline, idMap);
    }
    if (!this.cooking.isActive && offline.skill === 3 && offline.action !== null) {
      this.cooking.setFromOldOffline(offline, idMap);
    }
    if (!this.agility.isActive && offline.skill === 20 && offline.action !== null) {
      this.agility.setFromOldOffline(offline);
    }
    if (!this.astrology.isActive && offline.skill === 22 && offline.action !== null) {
...
class OfflineProgressElement extends HTMLElement {
  ...
  setMessages(game, oldSnapshot, newSnapshot, timeDiff, offlineAction) {
    ...
    game.currencies.forEach((currency) => {
      var _a, _b;
      const qtyDiff =
        ((_a = newSnapshot.currencies.get(currency)) !== null && _a !== void 0 ? _a : 0) -
        ((_b = oldSnapshot.currencies.get(currency)) !== null && _b !== void 0 ? _b : 0);
      if (qtyDiff !== 0) currencyDiffs.set(currency, qtyDiff);
    });
    ...
    newSnapshot.levels.forEach((level, skill) => {
      ...
      if (levelDiff > 0) levelGain.set(skill, levelDiff);
    });
    ...
    newSnapshot.abyssalLevels.forEach((level, skill) => {
      ...
      if (levelDiff > 0) abyssalLevelGain.set(skill, levelDiff);
    });
    ...
    newSnapshot.monsterKills.forEach((newKills, monster) => {
      var _a;
      const killDiff = newKills - ((_a = oldSnapshot.monsterKills.get(monster)) !== null && _a !== void 0 ? _a : 0);
      if (killDiff > 0) monstersKilled.set(monster, killDiff);
    });
    const dungeonsCompleted = new Map();
    newSnapshot.dungeonCompletion.forEach((completions, dungeon) => {
      var _a;
      const newComps =
        completions - ((_a = oldSnapshot.dungeonCompletion.get(dungeon)) !== null && _a !== void 0 ? _a : 0);
      if (newComps > 0) dungeonsCompleted.set(dungeon, newComps);
    });
    const tasksCompleted = new Map();
    newSnapshot.taskCompletions.forEach((completions, category) => {
      var _a;
      const newComps =
        completions - ((_a = oldSnapshot.taskCompletions.get(category)) !== null && _a !== void 0 ? _a : 0);
      if (newComps > 0) tasksCompleted.set(category, newComps);
    });
    ...
    newSnapshot.summoningMarks.forEach((newCount, mark) => {
      ...
      if (oldCount !== newCount) marksFound.set(mark, newCount - oldCount);
    });
    ...
    oldSnapshot.itemCharges.forEach((quantity, item) => {
      ...
      if (quantity > 0) itemChargesUsed.push({ item, quantity });
    });
    ...
    newSnapshot.cookingStockpile.forEach(({ item, quantity }, category) => {
      ...
      if (quantity > 0) stockPileItemsGained.push({ item, quantity });
    });
    ...
    oldSnapshot.food.forEach(({ item, quantity }, i) => {
      ...
      if (qtyDiff > 0) foodEaten.push({ item, quantity: qtyDiff });
      else if (qtyDiff < 0) {
    ...
    const getNodeDiff = (oldNode, newNode) => {
      return { totalFound: newNode.totalFound - oldNode.totalFound, hpFound: newNode.hpFound - oldNode.hpFound };
    };
    ...
    newSnapshot.ancientRelics.forEach((relicData, skill) => {
      relicData.forEach((count, relic) => {
        var _a, _b;
        const oldCount =
          (_b = (_a = oldSnapshot.ancientRelics.get(skill)) === null || _a === void 0 ? void 0 : _a.get(relic)) !==
            null && _b !== void 0
            ? _b
            : 0;
        if (count !== oldCount) {
          if (!ancientRelics.has(skill)) ancientRelics.set(skill, new Map());
    ...
    const currencyDiff = (diff, name, media) => {
      if (diff > 0) {
        return (
          templateLangString(`MENU_TEXT_CURRENCY_GAIN_${name}`, {
            curIcon: image(media),
        ...
        return (
          templateLangString(`MENU_TEXT_CURRENCY_LOSS_${name}`, {
            curIcon: image(media),
    ...
    xpGain.forEach((xpGain, skill) => {
      outputHeaders.push(
        templateString(getLangString('MISC_STRING_6'), {
    ...
    levelGain.forEach((levelGain, skill) => {
      ...
      outputHeaders.push(
        templateLangString(
          `MENU_TEXT_${levelGain === 1 ? 'LEVELED_UP_SKILL_ONCE' : 'LEVELED_UP_SKILL_TIMES'}`,
          templateData,
        ),
      );
    });
    abyssalXPGain.forEach((xpGain, skill) => {
      outputHeaders.push(
        templateString(getLangString('MENU_TEXT_ABYSSAL_SKILL_XP_GAINED'), {
          qty: posSpan(numberWithCommas(xpGain)),
          skillName: skill.name,
        }) +
          `${
    ...
    monstersKilled.forEach((killDiff, monster) => {
      if (killDiff > 0) {
        outputHeaders.push(
          templateLangString('MENU_TEXT_YOU_KILLED_MONSTER', {
            count: `${killDiff}`,
    ...
    dungeonsCompleted.forEach((countDiff, dungeon) => {
      if (countDiff > 0) {
        outputHeaders.push(
          templateLangString(`MENU_TEXT_${countDiff === 1 ? 'COMPLETED_DUNGEON_ONCE' : 'COMPLETED_DUNGEON_TIMES'}`, {
    ...
    marksFound.forEach((markCount, mark) => {
      if (markCount > 0) {
        outputHeaders.push(
          templateLangString('MENU_TEXT_YOU_FOUND_MARK', {
    ...
    itemsGained.forEach((qty, item) => {
      outputHeaders.push(
        templateLangString('MENU_TEXT_YOU_GAINED_ITEM', {
    ...
    newSnapshot.loot.forEach((qty, item) => {
      outputHeaders.push(
        templateLangString('MENU_TEXT_YOU_HAVE_ITEM_LOOT', {
          count: posSpan(numberWithCommas(qty)),
          itemImage: image(item.media),
          itemName: item.name,
        }),
    ...
    lostLoot.forEach((qty, item) => {
      outputHeaders.push(
        templateLangString('MENU_TEXT_LOST_ITEM_LOOT', {
          count: posSpan(numberWithCommas(qty)),
          itemImage: image(item.media),
          itemName: item.name,
        }),
    ...
    lostItems.forEach((qty, item) => {
      outputHeaders.push(
        templateLangString('MENU_TEXT_LOST_ITEM_BANK', {
          count: posSpan(numberWithCommas(qty)),
          itemImage: image(item.media),
          itemName: item.name,
        }),
    ...
    currencyDiffs.forEach((diff, currency) => {
      if (diff > 0) {
        outputHeaders.push(
          templateString(currency.gainTemplate, {
        ...
        outputHeaders.push(
          templateString(currency.usedTemplate, {
            curIcon: image(currency.media),
            count: negSpan(numberWithCommas(-diff)),
          }),
        );
    ...
    if (ppDiff !== 0) {
      outputHeaders.push(currencyDiff(ppDiff, 'PP', assets.getURI('assets/media/skills/prayer/prayer.png')));
    }
    if (spDiff !== 0) {
      outputHeaders.push(currencyDiff(spDiff, 'Soul_Points', assets.getURI('assets/media/bank/Lesser_Soul.png')));
    }
    itemsUsed.forEach((qty, item) => {
      outputHeaders.push(
        templateLangString('MENU_TEXT_ITEM_USAGE', {
    ...
    oldSnapshot.equipment.forEach((oldQuantity, slot) => {
      ...
      if (newQuantity === undefined || oldQuantity.item === game.emptyEquipmentItem) return;
      const quantityUsed =
        newQuantity.item === game.emptyEquipmentItem
    ...
    foodEaten.forEach(({ quantity, item }) => {
      outputHeaders.push(
        templateLangString('MENU_TEXT_FOOD_EATEN', {
    ...
    itemChargesUsed.forEach(({ item, quantity }) => {
      if (quantity > 0)
        outputHeaders.push(
          templateLangString('MENU_TEXT_GLOVE_CHARGE_USAGE', {
            count: negSpan(numberWithCommas(quantity)),
            itemImage: image(item.media),
            itemName: item.name,
    ...
    const addMiningNodeHeader = (nodeDiff, singular, plural, media) => {
      if (nodeDiff.totalFound > 0) {
        outputHeaders.push(
          templateLangString(`MENU_TEXT_${nodeDiff.totalFound === 1 ? singular : plural}`, {
    ...
    addMiningNodeHeader(
      ...
      'assets/media/skills/astrology/meteorite.png',
    );
    addMiningNodeHeader(onyxNode, 'ONYX_FOUND_ONE', 'ONYX_FOUND_MANY', 'assets/media/bank/onyx.png');
    addMiningNodeHeader(orichaNode, 'ORICHA_FOUND_ONE', 'ORICHA_FOUND_MANY', 'assets/media/bank/oricha.png');
    addMiningNodeHeader(ceruleanNode, 'CERULEAN_FOUND_ONE', 'CERULEAN_FOUND_MANY', 'assets/media/bank/cerulean.png');
    addMiningNodeHeader(abycite, 'ABYCITE_FOUND_ONE', 'ABYCITE_FOUND_MANY', 'assets/media/skills/mining/Abycite.png');
    addMiningNodeHeader(
      mysticite,
      'MYSTICITE_FOUND_ONE',
      'MYSTICITE_FOUND_MANY',
      'assets/media/skills/mining/Mysticite.png',
    );
    addMiningNodeHeader(
      echocite,
      'ECHOCITE_FOUND_ONE',
      'ECHOCITE_FOUND_MANY',
      'assets/media/skills/mining/Echocite.png',
    );
    addMiningNodeHeader(
      nightopalNode,
      'NIGHTOPAL_FOUND_ONE',
      'NIGHTOPAL_FOUND_MANY',
      'assets/media/skills/mining/Nightopal.png',
    );
    addMiningNodeHeader(
      shadowpearlNode,
      'SHADOWPEARL_FOUND_ONE',
      'SHADOWPEARL_FOUND_MANY',
      'assets/media/skills/mining/Shadowpearl.png',
    );
    addMiningNodeHeader(
      moonstoneNode,
      'MOONSTONE_FOUND_ONE',
      'MOONSTONE_FOUND_MANY',
      'assets/media/skills/mining/Moonstone.png',
    );
    addMiningNodeHeader(
      voidheartNode,
      'VOIDHEART_FOUND_ONE',
      'VOIDHEART_FOUND_MANY',
      'assets/media/skills/mining/Voidheart.png',
    );
    ancientRelics.forEach((relicData, skill) => {
      relicData.forEach((count, relic) => {
        outputHeaders.push(
          templateLangString(`ANCIENT_RELIC_LOCATED_COUNT`, {
    ...
    if (game.township.isUnlocked) {
      outputHeaders.push(
        negSpan(templateLangString('TOWNSHIP_MENU_OFFLINE_HEALTH', { value: `${newSnapshot.townshipHealth}` })),
      );
    }
    if (storageFull) {
