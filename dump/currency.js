...
class Currency extends NamespacedObject {
  ...
  queueNotification(amount) {
    ...
    this.game.combat.notifications.add({ type: 'Currency', args: [this, amount] });
  }
  renderAmount() {
  ...
  renderQuantities() {
    ...
    const highlight =
      (this.game.settings.formatNumberSetting === 0 && this._amount >= 10000000) ||
    ...
    elements.forEach((element) => {
      ...
      if (element.getAttribute('data-currency-highlight')) {
        ...
        if (highlight) element.classList.add('text-success');
        else element.classList.add('text-light');
      }
    });
  }
  renderTooltips() {
    ...
    if (tooltips.length > 0) updateTooltips();
  }
  getTooltipContent() {
  ...
  onAmountChange() {
    ...
    if (
      ((_b = (_a = this.game.openPage) === null || _a === void 0 ? void 0 : _a.action) === null || _b === void 0
        ? void 0
        : _b.queueCurrencyQuantityRender) !== undefined
    )
  ...
  decode(reader, version) {
    ...
    if (version >= 100) this.stats.decode(reader, version);
  }
...
class SlayerCoins extends Currency {
  ...
  getTooltipContent() {
    return `${this.formatAmount(numberWithCommas(this._amount))}<br><small class='text-warning'>${getLangString(
      'MENU_TEXT_SLAYER_COINS_FROM_TASKS',
    )}</small>`;
  }
  formatAmount(qtyText) {
