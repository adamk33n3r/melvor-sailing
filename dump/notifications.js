...
class NotificationsManager {
  ...
  createSkillXPNotification(skill, quantity) {
    ...
    if (quantity < 1 && quantity > 0) notification.quantity = Number.parseFloat(quantity.toFixed(2));
    else notification.quantity = Math.floor(quantity);
    notification.text = isOnMobileLayout
    ...
    if (skillNotification !== undefined) this.addNotification(skillNotification, notification);
  }
  newSkillXPNotification(skill) {
    if (this.skillXPNotificationClasses.has(skill)) return this.skillXPNotificationClasses.get(skill);
  ...
  createAbyssalXPNotification(skill, quantity) {
    ...
    notification.text = isOnMobileLayout
      ? getLangString('ABYSSAL_XP')
      : templateLangString('MENU_TEXT_AXP_SKILL_NAME', { skillName: skill.name });
    notification.isImportant = false;
    notification.isError = false;
    const abyssalNotification = this.newAbyssalXPNotification(skill);
    if (abyssalNotification !== undefined) this.addNotification(abyssalNotification, notification);
  }
  newAbyssalXPNotification(skill) {
    if (this.abyssalXPNotificationClasses.has(skill)) return this.abyssalXPNotificationClasses.get(skill);
  ...
  createMasteryLevelNotification(action, level) {
    ...
    if (masteryLevelNotification !== undefined) this.addNotification(masteryLevelNotification, notification);
  }
  newMasteryLevelNotification(action) {
    if (this.masteryLevelNotificationClasses.has(action)) return this.masteryLevelNotificationClasses.get(action);
  ...
  createItemNotification(item, quantity) {
    ...
    if (itemNotification !== undefined) this.addNotification(itemNotification, notification);
  }
  newAddItemNotification(item) {
    if (this.addItemNotificationClasses.has(item)) return this.addItemNotificationClasses.get(item);
  ...
  newRemoveItemNotification(item) {
    if (this.removeItemNotificationClasses.has(item)) return this.removeItemNotificationClasses.get(item);
  ...
  createCurrencyNotification(currency, quantity) {
    ...
    const currencyNotification =
      quantity > 0 ? this.getAddCurrencyNotification(currency) : this.getRemoveCurrencyNotification(currency);
  ...
  createGPNotification(quantity) {
    ...
    const gpNotification =
      quantity > 0 ? this.newAddGenericNotification('AddGP') : this.newRemoveGenericNotification('RemoveGP');
    if (gpNotification !== undefined) this.addNotification(gpNotification, notification);
  }
  createSlayerCoinsNotification(quantity) {
    ...
    const scNotification =
      quantity > 0
        ? this.newAddGenericNotification('AddSlayerCoins')
        : this.newRemoveGenericNotification('RemoveSlayerCoins');
    if (scNotification !== undefined) this.addNotification(scNotification, notification);
  }
  createSummoningMarkNotification(mark) {
    ...
    if (markNotification !== undefined) this.addNotification(markNotification, notification);
  }
  createErrorNotification(customID, msg) {
    ...
    if (errorNotification !== undefined) this.addNotification(errorNotification, notification);
  }
  createSuccessNotification(customID, msg, media, quantity = 1) {
    ...
    if (successNotification !== undefined) this.addNotification(successNotification, notification);
  }
  createInfoNotification(customID, msg, media, quantity = 1) {
    ...
    if (infoNotification !== undefined) this.addNotification(infoNotification, notification);
  }
  newAddGenericNotification(type) {
    if (this.genericNotificationClasses.has(type)) return this.genericNotificationClasses.get(type);
  ...
  newSummoningMarkNotification(mark) {
    if (this.summoningMarkNotificationClasses.has(mark)) return this.summoningMarkNotificationClasses.get(mark);
  ...
  sortNotifications() {
    const sorted = [...this.activeNotifications.entries()].sort((a, b) => {
      if (a[1].isError && !b[1].isError) return -1;
      if (!a[1].isError && b[1].isError) return 1;
      if (a[1].isImportant && !b[1].isImportant) return -1;
      if (!a[1].isImportant && b[1].isImportant) return 1;
      return 0;
    });
  ...
  editNotification(key, notification) {
    ...
    if (existingNotification) {
      ...
      key.type === 'MasteryLevel'
        ? (updatedNotification.quantity = notification.quantity)
        : (updatedNotification.quantity += notification.quantity);
  ...
  updateNotificationElement(key, notification, qtyChange) {
    ...
    if (notificationElement) {
      ...
      (_a = notificationElement.splashManager) === null || _a === void 0
        ? void 0
        : _a.add({ source: source, amount: qtyChange, xOffset: 100 });
  ...
  adjustAllNotificationPositions() {
    ...
    this.activeNotifications.forEach((_, key) => {
      ...
      totalHeight +=
        (_a = childNode === null || childNode === void 0 ? void 0 : childNode.offsetHeight) !== null && _a !== void 0
          ? _a
          : 0;
      pos++;
    });
...
class GameNotificationElement extends HTMLElement {
  ...
  setNotification(key, notification, game) {
    ...
    if (game.settings.showItemNamesInNotifications) this.setText(notification.text);
    this.toggleCompact();
  }
  ...
  setQuantity(notification, type) {
    if (notification.quantity === 0) this.divQuantity.classList.add('d-none');
    else this.divQuantity.classList.remove('d-none');
    ...
    if (notification.isError) {
      for (let i = 3; i < notification.quantity; i += 3) {
        if (i % 12 === 0) qty = `${textSymbol}${numberWithCommas(notification.quantity)}`;
        qty += `?`;
      }
    }
