  ...
  constructor(namespace, data, game) {
    ...
    if (data.allowedGamemodeIDs !== undefined) {
      data.allowedGamemodeIDs.forEach((id) => {
        ...
        if (mode === undefined) throw new UnregisteredConstructionError(this, Gamemode.name, id);
...
class ShopPurchase extends NamespacedObject {
  constructor(namespace, data, game) {
    ...
    try {
      ...
      if (data.contains.bankTab) this.contains.bankTab = data.contains.bankTab;
  ...
  get description() {
    ...
    if (this.hasDisabledModifier) desc += getLangString('MENU_TEXT_CONTAINS_DISABLED_MODIFIER');
    return desc !== '' ? desc : 'Error: No Purchase Description';
  }
  get hasDisabledModifier() {
    return (
      this.contains.stats !== undefined &&
  ...
  get purchaseRequirements() {
    var _a;
    return (_a = this._purchaseRequirements.get(game.currentGamemode)) !== null && _a !== void 0
      ? _a
      : this._defaultPurchaseRequirements;
  }
  ...
  getTemplatedDescription(shop) {
    return templateString(
      this.description,
      this.getDescriptionTemplateData(shop.capPurchaseQuantity(this, shop.buyQuantity)),
    );
  }
  ...
  getCostsFromData(data, game) {
    ...
    if (data.gp && !isShopCostZero(data.gp)) currencies.push(Object.assign(data.gp, { currency: game.gp }));
    ...
    return { currencies, items: game.items.getQuantities(data.items) };
  }
}
...
class ShopUpgradeChain extends NamespacedObject {
  constructor(namespace, data, game) {
    super(namespace, data.id);
    this._defaultMedia = 'assets/media/main/question.png';
    try {
      ...
      if (data.defaultMedia !== undefined) this._defaultMedia = data.defaultMedia;
    } catch (e) {
      throw new DataConstructionError(ShopUpgradeChain.name, e, this.id);
    }
  }
  get chainName() {
    if (this.chainNameLang !== undefined) return getLangString(this.chainNameLang);
    return this._chainName;
  }
  get defaultName() {
    if (this.nameLang !== undefined) return getLangString(this.nameLang);
    return this._defaultName;
  }
  get defaultDescription() {
    if (this.descriptionLang !== undefined) return getLangString(this.descriptionLang);
...
class Shop extends GameEventEmitter {
  ...
  renderUpgrades() {
    ...
    if (this.game.modifiers.treeCutLimit > 0) $('#skill-woodcutting-multitree').removeClass('d-none');
    ...
    this.upgradeChains.forEach((chain) => {
      ...
      const upgradeName =
        (_a = lowestPurchase === null || lowestPurchase === void 0 ? void 0 : lowestPurchase.name) !== null &&
        _a !== void 0
          ? _a
          : chain.defaultName;
      const upgradeImg =
        (_b = lowestPurchase === null || lowestPurchase === void 0 ? void 0 : lowestPurchase.media) !== null &&
        _b !== void 0
          ? _b
          : chain.defaultMedia;
      let upgradeDesc;
  ...
  getTotalStatsInChain(purchase) {
    ...
    while (purchase !== undefined) {
      if (purchase.contains.stats !== undefined) summary.addStatObject(purchase, purchase.contains.stats);
  ...
  buyItemOnClick(purchase, confirmed = false) {
    ...
    if (!this.game.tutorial.complete && !this.game.tutorial.allowedShopPurchases.has(purchase)) {
      SwalLocale.fire({
        ...
        html: `<h5 class="font-w400 text-combat-smoke font-size-sm"><img class="skill-icon-xs mr-1" src="${
          purchase.media
        }"> ${getLangString('TUTORIAL_SHOP_LOCKED')}</h5>
    ...
    if (savePurchase) {
      ...
      if (this.game.harvesting !== undefined) this.game.harvesting.renderQueue.veinUnlock = true;
      (_a = this.game.archaeology) === null || _a === void 0
        ? void 0
    ...
    if (purchase.contains.stats !== undefined) this.computeProvidedStats();
  ...
  getCurrencyCost(cost, buyQuantity, boughtQuantity) {
    ...
    switch (cost.type) {
      case 'Linear':
        return (
          (cost.scaling / 2) * (newQuantity * (newQuantity - 1) - boughtQuantity * (boughtQuantity - 1)) +
  ...
  getCurrentDescription(purchase) {
    if (purchase.currentDescription !== undefined && purchase.contains.stats !== undefined) {
      ...
      if (stats.modifiers !== undefined) {
        ...
        if (firstApplicator.conditionChances.length > 0) value = firstApplicator.conditionChances[0].chance;
      }
      ...
      switch (purchase.currentDescription) {
        ...
        case 'SecondsDecrease':
          return templateLangString('MENU_TEXT_CURRENTLY_SECONDS_DEC', { seconds: formatFixed(value / 1000, 1) });
        case 'SecondsIncrease':
          return templateLangString('MENU_TEXT_CURRENTLY_SECONDS_INC', { seconds: formatFixed(value / 1000, 1) });
      }
    }
    return '';
  }
  convertFromOldFormat(save, idMap) {
    if (save.shopItemsPurchased !== undefined) {
      save.shopItemsPurchased.forEach((purchasedItem, oldPurchaseID) => {
        ...
        if (purchase === undefined) purchase = this.purchases.getDummyObject(newID, DummyShopPurchase, this.game);
        if (
          (save.version > 17 && purchase.category.id === 'melvorD:GolbinRaid') ||
          purchase.category.id === 'melvorF:Township'
        ) {
          purchasedItem.quantity = Math.min(purchasedItem.quantity, purchase.getBuyLimit(this.game.currentGamemode));
        }
        if (newID === 'melvorD:GolbinCrate') purchasedItem.quantity = this.game.golbinRaid.cratesPurchased;
