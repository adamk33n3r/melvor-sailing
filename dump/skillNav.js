...
class SkillProgressDisplay {
  constructor(game) {
    ...
    game.skills.forEach((skill) => {
      ...
      if (skill.hasAbyssalLevels) {
        ...
        if (abyssalLevelEl !== null) elems.abyssalLevel.push(abyssalLevelEl);
        if (abyssalPercentEl !== null) elems.abyssalPercent.push(abyssalPercentEl);
        if (abyssalXpEl !== null) elems.abyssalXp.push(abyssalXpEl);
        if (abyssalProgressEl !== null) elems.abyssalProgress.push(abyssalProgressEl);
      }
  ...
  getSkillElements(skill) {
    ...
    if (skillElems === undefined) throw new Error(`Tried to update skill progress for invalid skill: ${skill.id}`);
    return skillElems;
  }
  ...
  createTooltipHTML(skill) {
    ...
    let xpText;
    if (level >= skill.maxLevelCap) xpText = numberWithCommas(Math.floor(xp));
    else xpText = numberWithCommas(Math.floor(xp)) + ' / ' + numberWithCommas(exp.levelToXP(level + 1));
  ...
  createAyssalTooltipHTML(skill) {
    ...
    let xpText;
    if (level >= skill.maxAbyssalLevelCap) xpText = numberWithCommas(Math.floor(xp));
    else xpText = numberWithCommas(Math.floor(xp)) + ' / ' + numberWithCommas(abyssalExp.levelToXP(level + 1));
    return `<div class='text-center'>${xpText}</div>`;
  }
}
...
class SkillNav {
      ...
      this.navs.set(
        ...
        pages.map((page) => {
          const category = sidebar.category(
            page.skillSidebarCategoryID || (page.id === 'melvorD:Combat' ? 'Combat' : 'Non-Combat'),
          );
          const navItem = category.item(skill.id);
          const navAside = navItem.asideEl;
          if (navAside === undefined) throw new Error('Error creating SkillNav, sidebar item does not have aside.');
          const aside = createElement('skill-sidebar-aside', { className: 'justify-vertical-right', parent: navAside });
          const nav = { item: navItem, name: navItem.nameEl, aside };
          return nav;
        }),
      );
    });
  ...
  getNavs(skill) {
    ...
    if (navs === undefined) throw new Error(`Tried to update skill nav for invalid skill: ${skill.id}`);
    return navs;
  }
  shouldShowStandardLevels(skill) {
    return this.game.settings.sidebarLevels !== 2 && skill.shouldShowStandardLevels;
  }
  shouldShowAbyssalLevels(skill) {
    return this.game.settings.sidebarLevels !== 1 && skill.shouldShowAbyssalLevels;
  }
...
class SkillHeaderElement extends HTMLElement {
  constructor() {
    ...
    this.abyssalLevelCapButton = getElementFromFragment(
      this._content,
      'abyssal-level-cap-button',
      'level-cap-purchase-button',
    );
  ...
  updateAbyssalXP(game, skill) {
    ...
    if (level < skill.currentAbyssalLevelCap) {
      this.abyssalXp.textContent = `${numberWithCommas(Math.floor(xp))} / ${numberWithCommas(
        abyssalExp.levelToXP(level + 1),
      )}`;
    } else {
  ...
  toggleCombatLevelCap(show) {
    if (show) showElement(this.combatLevelXpLimit);
    else hideElement(this.combatLevelXpLimit);
  }
  updateLevelVisibility(skill) {
...
class CombatSkillProgressTableRow {
  ...
  updateXP(game, skill) {
    ...
    this.skillProgressBar.style.width = `${progress}%`;
    if (this.skillXPTooltip === undefined) this.skillXPTooltip = this.createXPTooltip(this.skillProgressBarContainer);
  ...
  createXPTooltip(element) {
    return tippy(element, { allowHTML: true, placement: 'top', interactive: false, animation: false });
  }
}
