  ...
  getPkgObjects(pkg, type) {
    ...
    switch (type) {
      ...
      case ScopeSourceType.Subcategory:
        return (
          (_e =
            (_d = (_c = pkg.data) === null || _c === void 0 ? void 0 : _c.skillData) === null || _d === void 0
              ? void 0
              : _d.find((s) => s.skillID === 'melvorD:Magic')) === null || _e === void 0
            ? void 0
            : _e.data
        ).spellCategories;
    }
...
class CombatAreaScopeSource extends NamespacedObject {
  ...
  getPkgObjects(pkg, type) {
    if (pkg.data === undefined) return;
    switch (type) {
      case ScopeSourceType.Category:
        return pkg.data.combatAreaCategories;
      case ScopeSourceType.Action: {
        const objects = [];
        if (pkg.data.combatAreas !== undefined) objects.concat(pkg.data.combatAreas);
        if (pkg.data.slayerAreas !== undefined) objects.concat(pkg.data.slayerAreas);
        if (pkg.data.dungeons !== undefined) objects.concat(pkg.data.dungeons);
        if (pkg.data.abyssDepths !== undefined) objects.concat(pkg.data.abyssDepths);
        if (pkg.data.strongholds !== undefined) objects.concat(pkg.data.strongholds);
...
class ModifierScope {
  constructor(scope) {
    if (scope !== undefined) ModifierScope.copyScope(scope, this);
  }
  ...
  static getScopefromData(data, game, scopeSource) {
    ...
    try {
      ...
      if (data.categoryID !== undefined) {
        if (scopeSource === undefined) throw new Error('The category scope requires a skill/source for the scope.');
      ...
      if (data.actionID !== undefined) {
        if (scopeSource === undefined) throw new Error('The action scope requires a skill/source for the scope.');
        scope.action = this.getObjectFromSourceSafe(scopeSource, ScopeSourceType.Action, data.actionID);
      }
      if (data.subcategoryID !== undefined) {
        if (scopeSource === undefined) throw new Error('The subcategory scope requires a skill/source for scope.');
  ...
  static getObjectFromSourceSafe(source, type, id) {
    ...
    if (registry === undefined) throw new Error(`Skill/Scope source ${source.name} does not support ${type}`);
  ...
  static copyScope(from, to) {
    if (from.skill !== undefined) to.skill = from.skill;
    if (from.damageType !== undefined) to.damageType = from.damageType;
    if (from.realm !== undefined) to.realm = from.realm;
    if (from.currency !== undefined) to.currency = from.currency;
    if (from.category !== undefined) to.category = from.category;
    if (from.action !== undefined) to.action = from.action;
    if (from.subcategory !== undefined) to.subcategory = from.subcategory;
    if (from.item !== undefined) to.item = from.item;
    if (from.effectGroup !== undefined) to.effectGroup = from.effectGroup;
  }
  ...
  static getScopeKeyFromData(data) {
    let key = 0;
    if (data.skillID !== undefined) key += this.SCOPE_BIT_FIELD.skill;
    if (data.damageTypeID !== undefined) key += this.SCOPE_BIT_FIELD.damageType;
    if (data.realmID !== undefined) key += this.SCOPE_BIT_FIELD.realm;
    if (data.currencyID !== undefined) key += this.SCOPE_BIT_FIELD.currency;
    if (data.categoryID !== undefined) key += this.SCOPE_BIT_FIELD.category;
    if (data.actionID !== undefined) key += this.SCOPE_BIT_FIELD.action;
    if (data.subcategoryID !== undefined) key += this.SCOPE_BIT_FIELD.subcategory;
    if (data.itemID !== undefined) key += this.SCOPE_BIT_FIELD.item;
    if (data.effectGroupID !== undefined) key += this.SCOPE_BIT_FIELD.effectGroup;
    return key;
  }
}
...
class ModifierValue extends ModifierScope {
  ...
  getDescription(negMult = 1, posMult = 1, precision = 2) {
    const { template, includeSign, inverted } = this.modifier.getValueTemplateString(this);
    ...
    return { description, isNegative };
  }
  print(negMult = 1, posMult = 1, precision = 2) {
    const { description, isNegative } = this.getDescription(negMult, posMult, precision);
    return { text: description, isNegative, isDisabled: this.modifier.disabled };
  }
  ...
  getTemplatedDescription(prefix, postfix) {
    const { template, includeSign } = this.modifier.getValueTemplateString(this);
  ...
  static encode(writer, value) {
    ...
    if (value.skill !== undefined) writer.writeNamespacedObject(value.skill);
    if (value.damageType !== undefined) writer.writeNamespacedObject(value.damageType);
    if (value.realm !== undefined) writer.writeNamespacedObject(value.realm);
    if (value.currency !== undefined) writer.writeNamespacedObject(value.currency);
    if (value.category !== undefined) writer.writeNamespacedObject(value.category);
    if (value.action !== undefined) writer.writeNamespacedObject(value.action);
    if (value.subcategory !== undefined) writer.writeNamespacedObject(value.subcategory);
    if (value.item !== undefined) writer.writeNamespacedObject(value.item);
    if (value.effectGroup !== undefined) writer.writeNamespacedObject(value.effectGroup);
    return writer;
  }
  static decode(game, reader, version) {
    ...
    if (typeof modifier === 'string' || failed) return undefined;
  ...
  static fromAlias(modifier, valueData, alias, game) {
    ...
    if (!modValue.validateScope())
      throw new Error(
        `Error constructing ${ModifierValue.name}. ${Modifier.name} with id: ${modValue.modifier.id} does not support scoping with: ${Object.keys(scope)}`,
      );
    return modValue;
  }
  ...
  static fromKeySafe(key, value, game) {
    try {
      return this.fromKey(key, { value }, game);
    } catch (_a) {
      return undefined;
    }
  }
  static fromSkillKeyValues(key, values, game) {
    ...
    if (alias !== undefined) {
      values.forEach(({ skill, value }) => {
        if (alias.isNegative) value = -value;
        const newValue = new ModifierValue(alias.modifier, value, { skill });
        ModifierScope.copyScope(alias.alias, newValue);
        if (newValue.validateScope()) modValues.push(newValue);
      });
    } else {
      const modifier = game.modifierRegistry.getObjectByID(Modifier.getIdFromKey(key));
      if (modifier !== undefined) {
        values.forEach(({ skill, value }) => {
          const newValue = new ModifierValue(modifier, value, { skill });
          if (newValue.validateScope()) modValues.push(newValue);
        });
      }
    }
...
class ModifierDescription {
  constructor(data, game, scopeSource) {
    ...
    try {
      ...
      if (data.lang !== undefined) this._lang = data.lang;
      if (data.above !== undefined) this.range[0] = data.above;
      if (data.below !== undefined) this.range[1] = data.below;
      if (data.includeSign !== undefined) this.includeSign = data.includeSign;
      if (data.inverted !== undefined) this.inverted = data.inverted;
      if (data.scope !== undefined) this.scope = ModifierScope.getScopefromData(data.scope, game, scopeSource);
    } catch (e) {
      throw new DataConstructionError(ModifierDescription.name, e);
    }
  }
  get template() {
    if (this._lang !== undefined) return getLangString(this._lang);
    return this._text;
  }
}
class ModifierScoping {
  constructor(data, game) {
    ...
    try {
      ...
      if (data.posAliases !== undefined) this.posAliases = this.constructAliases(scopeKey, game, data.posAliases);
      if (data.negAliases !== undefined) this.negAliases = this.constructAliases(scopeKey, game, data.negAliases);
    } catch (e) {
  ...
  getDescriptionTemplate(modValue) {
    const description = this.descriptions.find((desc) => {
      return (
        (desc.scope === undefined || ModifierTable.doesQueryMatchScope(desc.scope, modValue)) &&
  ...
  addDescriptions(data, game) {
    data.forEach((descData) => {
      ...
      if (description.scope !== undefined && !this.isScopeCombatible(description.scope)) {
        throw new Error(
          `Description with scope ${ModifierTable.getQueryKey(description.scope)} is not compatible with scope ${ModifierTable.getQueryKey(this.scopes)}.`,
        );
      }
  ...
  sortDescriptions() {
    this.descriptions.sort((a, b) => {
      ...
      else if (a.range[0] === b.range[0]) return 0;
      else return -1;
    });
  }
}
class Modifier extends NamespacedObject {
  constructor(namespace, data, game) {
    ...
    try {
      if (data.inverted !== undefined) this.inverted = data.inverted;
      if (data.allowPositive !== undefined) this.allowPositive = data.allowPositive;
      if (data.allowNegative !== undefined) this.allowEnemy = data.allowNegative;
      if (data.isCombat !== undefined) this.isCombat = data.isCombat;
      if (data.allowEnemy !== undefined) this.allowEnemy = data.allowEnemy;
      ...
      this.allowedScopes = data.allowedScopes.map((d) => {
        ...
        if (this.scopeMap.has(key))
          console.warn(
            `${Modifier.name} with id "${this.id}" has multiple entries for scope ${Object.keys(scoping.scopes)}`,
          );
    ...
    try {
      data.allowedScopes.forEach((scoping) => {
        ...
        if (existing !== undefined) {
          ...
          if (DEBUGENABLED) console.warn(`Modifications creating new scoping for modifier ${this.id}`);
  ...
  getValueTemplateString(modValue) {
    ...
    if (scoping === undefined) scoping = this.scopeMap.get(scopeKey);
    if (scoping === undefined)
      return { template: 'Error: No scoping exists for value.', includeSign: true, inverted: false };
  ...
  getScopingFromDataWithAlias(data, alias) {
    ...
    if (scoping !== undefined) return scoping;
    throw new Error(
      `No scoping exists for scope: ${Object.keys(data)} for modifier "${this.id}" with alias "${alias.key}"`,
    );
  }
  hasScoping(scope) {
  ...
  static getScopeKey(scope) {
    ...
    for (let i = 0; i < scopeArray.length; i++) {
      if (scopeArray[i] !== undefined) key += Math.pow(2, i);
    }
    return key;
  }
  static getIdFromKey(key) {
    return key.includes(':') ? key : `${'melvorD'}:${key}`;
  }
}
...
class ModifierRegistry extends NamespaceRegistry {
  ...
  registerObject(modifier) {
    ...
    if (modifier.allowEnemy) this.enemy.registerObject(modifier);
    this.updateAliases(modifier);
  }
  updateAliases(modifier) {
    modifier.allowedScopes.forEach((scoping) => {
      var _a, _b;
      (_a = scoping.posAliases) === null || _a === void 0
        ? void 0
        : _a.forEach((alias) => {
            const aliasInfo = { isNegative: false, modifier, alias };
            this.aliases.set(alias.key, aliasInfo);
            if (modifier.allowEnemy) this.enemyAliases.set(alias.key, aliasInfo);
          });
      (_b = scoping.negAliases) === null || _b === void 0
        ? void 0
        : _b.forEach((alias) => {
            const aliasInfo = { isNegative: true, modifier, alias };
            this.aliases.set(alias.key, aliasInfo);
            if (modifier.allowEnemy) this.enemyAliases.set(alias.key, aliasInfo);
          });
    });
