...
const COMBAT_LEVEL_KEYS = ['Hitpoints', 'Attack', 'Strength', 'Defence', 'Ranged', 'Magic', 'Prayer', 'Corruption'];
let shopMenu;
let tutorialMenus;
let combatMenus;
const combatAreaMenus = new CombatAreaMenuManager();
const resistanceMenus = { viewItemStats: new Map(), viewEquipmentStats: new Map(), itemUpgrade: new Map() };
...
function initMenus() {
  ...
  if (checkMediaQuery('(max-width: 991px)')) shiftToMobileLayout();
  ...
  playerHTMLElements = {
    ...
    golbinLevels: {
      ...
      Corruption: document.getElementById('golbin-raid-prayer-level-text'),
    },
    barrier: [],
    barrierBar: [],
    maxBarrier: [],
  ...
  game.damageTypes.forEach((damageType) => {
    ...
    if (itemViewResistEl === null) throw new Error('Item view resistances div element not found');
    ...
    if (itemViewCurrentResistEl === null) throw new Error('Item view current resistances div element not found');
  ...
  combatMenus = {
    ...
    playerEffectRenderer: new EffectRenderer(
      document.getElementById('combat-player-effect-icon-container'),
      document.getElementById('combat-player-effect-progress-bar-container'),
      document.getElementById('combat-player-effect-progress-icon-container'),
    ),
    enemyEffectRenderer: new EffectRenderer(
      document.getElementById('combat-enemy-effect-icon-container'),
      document.getElementById('combat-enemy-effect-progress-bar-container'),
      document.getElementById('combat-enemy-effect-progress-icon-container'),
    ),
  ...
  combatAreaCategoryMenu.addOptions(game.combatAreaCategoryOrder, getLangString('SELECT_COMBAT_AREA'), (category) =>
    combatAreaMenus.toggleCategory(category),
  );
  ...
  astrologyMenus = { constellations, locked: lockedConstellation, infoPanel, explorePanel };
  ...
  herbloreCategoryMenu.addOptions(
    game.herblore.categories.allObjects,
    getLangString('MENU_TEXT_SELECT_HERBLORE_CATEGORY'),
  ...
  smithingCategoryMenu.addOptions(
    game.smithing.categories.allObjects,
    getLangString('MENU_TEXT_SELECT_SMITHING_CATEGORY'),
  ...
  runecraftingCategoryMenu.addOptions(
    game.runecrafting.categories.allObjects,
    getLangString('MENU_TEXT_SELECT_RUNECRAFTING_CATEGORY'),
  ...
  craftingCategoryMenu.addOptions(
    game.crafting.categories.allObjects,
    getLangString('MENU_TEXT_SELECT_CRAFTING_CATEGORY'),
  ...
  fletchingCategoryMenu.addOptions(
    game.fletching.categories.allObjects,
    getLangString('MENU_TEXT_SELECT_FLETCHING_CATEGORY'),
  ...
  summoningCategoryMenu.addOptions(
    game.summoning.categories.allObjects,
    getLangString('MENU_TEXT_SELECT_SUMMONING_CATEGORY'),
  ...
  game.summoning.categories.forEach((category) => {
    if (category.type !== 'Tablet') return;
