...
class CombatManager extends BaseManager {
  ...
  get media() {
    return assets.getURI('assets/media/skills/combat/combat.png');
  }
  get name() {
    return getLangString('PAGE_NAME_Combat');
  }
  get activeSkills() {
    if (!this.isActive || this.areaType === CombatAreaType.None) return [];
  ...
  get areaRealm() {
    if (this.selectedArea === undefined) return this.game.defaultRealm;
    return this.selectedArea.realm;
  }
  get inAbyssalArea() {
    return this.selectedArea !== undefined && this.selectedArea.realm.id === 'melvorItA:Abyssal';
  }
  get combatTriangle() {
    var _a, _b;
    const triangleSet =
      (_b = (_a = this.selectedArea) === null || _a === void 0 ? void 0 : _a.combatTriangleSet) !== null &&
      _b !== void 0
        ? _b
        : this.game.normalCombatTriangleSet;
    return triangleSet[this.game.currentGamemode.combatTriangleType];
  }
  get combatTriangleSet() {
    var _a, _b;
    return (_b = (_a = this.selectedArea) === null || _a === void 0 ? void 0 : _a.combatTriangleSet) !== null &&
      _b !== void 0
      ? _b
      : this.game.normalCombatTriangleSet;
  }
  ...
  get isFightingITMBoss() {
    if (!(this.selectedArea instanceof Dungeon)) return false;
    return (
      this.selectedArea.id === 'melvorF:Into_the_Mist' && this.areaProgress > this.selectedArea.monsters.length - 4
    );
  }
  get onSlayerTask() {
    return (
  ...
  get areaRequirementsMet() {
    if (this.selectedArea === undefined) return true;
    if (
    ...
    if (this.selectedArea instanceof SlayerArea) slayerLevelReq = this.selectedArea.slayerLevelRequired;
  ...
  setDungeonCompleteCount(dungeon, count) {
    if (count <= 0) this.dungeonCompletion.delete(dungeon);
    else this.dungeonCompletion.set(dungeon, count);
  }
  getMonsterDropsHTML(monster, respectArea) {
    let drops = '';
    if (
      monster.lootChance > 0 &&
      ...
      drops = monster.lootTable.sortedDropsArray
        .map((drop) => {
          ...
          if (DEBUGENABLED)
            dropText += ` (${((monster.lootChance * drop.weight) / monster.lootTable.weight).toFixed(2)}%) (1 in ${(
              1 /
              ((monster.lootChance * drop.weight) / monster.lootTable.weight / 100)
            ).toFixed(1)})`;
          return dropText;
        })
        .join('<br>');
    ...
    let bones = '';
    const dropsBones =
      monster.bones !== undefined && !(respectArea && this.selectedArea !== undefined && !this.selectedArea.dropsBones);
  ...
  getAreaEffectMagnitude(areaEffect, realm) {
    let bonus = 0;
    if (this.isEventActive && this.eventPassives.some((passive) => passive.id === 'melvorF:EventPassive12')) {
      bonus = 5;
    }
  ...
  initialize() {
    ...
    if (this.isEventActive) {
      if (this.eventPassivesBeingSelected.size > 0) this.fireEventPassiveModal();
  ...
  getEquipmentRestrictions() {
    ...
    if (this.isActive && this.selectedArea !== undefined) {
      ...
      if (area instanceof SlayerArea) {
        ...
        if (reqs.length > 0) {
          restrictions.slayer = { level: area.slayerLevelRequired, items: reqs.map((req) => req.item) };
        }
...
Current Area ID: ${(_a = this.selectedArea) === null || _a === void 0 ? void 0 : _a.id}`;
  ...
  render() {
    if (this.renderQueue.slayerAreaEffects) this.renderSlayerAreaEffects();
  ...
  renderSlayerAreaEffects() {
    if (this.renderQueue.slayerAreaEffects) combatAreaMenus.all.forEach((menu) => menu.updateAreaEffectValues());
    if (this.areaType === CombatAreaType.Slayer) this.renderQueue.location = true;
    this.renderQueue.slayerAreaEffects = false;
  }
  renderEventMenu() {
    if (!this.renderQueue.eventMenu) return;
    if (this.isEventActive) {
  ...
  renderAreaRequirements() {
    if (!this.renderQueue.areaRequirements) return;
  ...
  renderResistanceMenus() {
    if (!this.renderQueue.resistanceMenus) return;
  ...
  renderCorruptionMenus() {
    var _a;
    if (!this.renderQueue.corruptionMenus) return;
    ...
    if (shouldShow) {
      ...
      showElement(combatMenus.menuTabs[7]);
    } else {
      hideElement(combatMenus.corruptionSettings);
      hideElement(combatMenus.menuTabs[7]);
    }
    if (
      localStorage.getItem('corruptionMenuGlow') !== null ||
  ...
  renderAreaSkillUnlockCounts() {
    if (!this.renderQueue.areaSkillUnlock) return;
  ...
  onPlayerDeath() {
    ...
    if (this.isEventActive) this.stopEvent();
    ...
    if (this.game.currentGamemode.isPermaDeath && shouldDeleteSave) {
      ...
      if (connectedToSteam) unlockSteamAchievement('NEW_ACHIEVEMENT_2_29', 61);
      resetAccountData();
    }
  ...
  onEnemyDeath() {
    var _a;
    if (this.enemy.monster === undefined) throw new Error('Enemy died, but has no monster set.');
    const killedEvent = new MonsterKilledEvent(
      this.enemy.monster,
      this.player.attackType,
      this.player,
      this.enemy.isCorrupted,
    );
    if (!loadingOfflineProgress) this.game.telemetry.createMonsterKillEvent(this.enemy.monster);
      ...
      if (this.areaProgress === this.eventDungeonLength) {
        ...
        if (this.activeEventAreas.size === 0) {
          ...
          this.bank.addItem(
            ...
            false,
            true,
            `Dungeon.${(_a = this.selectedArea) === null || _a === void 0 ? void 0 : _a.id}`,
          );
          this.increaseEventProgress(this.activeEvent);
        }
      }
    } else {
      if (this.selectedArea !== undefined) this.rewardForEnemyDeath(this.enemy.monster, this.selectedArea);
      if (this.selectedArea instanceof AbyssDepth) {
  ...
  retroactivelyAddOneTimeRewards() {
    this.game.dungeons.forEach((dungeon) => {
      ...
      if (
        dungeon.oneTimeReward !== undefined &&
  ...
  rewardForEnemyDeath(monster, area) {
    ...
    if (!receiveNoItemDrops) {
      if (area.dropsBones) this.dropEnemyBones(monster);
      if (area.dropsLoot) {
    ...
    if (area.dropsCurrency) this.dropEnemyCurrency(monster);
    let slayerXPReward = 0;
    let slayerAXPReward = 0;
    if (this.areaType === CombatAreaType.Slayer && this.player.damageType.id !== 'melvorItA:Eternal') {
      if (this.inAbyssalArea) slayerAXPReward += this.enemy.stats.maxHitpoints / numberMultiplier;
      else slayerXPReward += this.enemy.stats.maxHitpoints / numberMultiplier / 2;
    }
    if (this.onSlayerTask) {
      ...
      if (this.slayerTask.isAbyssal) slayerAXPReward += this.enemy.stats.maxHitpoints / numberMultiplier;
      else slayerXPReward += this.enemy.stats.maxHitpoints / numberMultiplier;
    }
    if (slayerXPReward > 0) {
    ...
    if (this.enemy.isCorrupted && this.player.damageType.id !== 'melvorItA:Eternal') {
      (_a = this.game.corruption) === null || _a === void 0
        ? void 0
        : _a.addAbyssalXP(Math.floor(400 + (monster.levels.Hitpoints * monster.levels.Corruption) / 25));
    }
  }
  getLootDoublingChance(monster) {
    ...
    chance += this.player.modifiers.getValue('melvorD:doubleItemsChanceAgainstDamageType', monster.damageType.modQuery);
    return chance;
  }
  dropEnemyLoot(monster) {
    ...
    if (rollPercentage(monster.lootChance)) {
      ...
      if (rollPercentage(this.getLootDoublingChance(monster))) quantity *= 2;
      const autoLooted =
        this.player.modifiers.autoLooting &&
      ...
      if (autoLooted) {
        ...
        let stack = false;
        if (this.player.modifiers.allowLootContainerStacking > 0) stack = true;
  ...
  dropBarrierDust(monster) {
    if (!monster.hasBarrier) return;
    const barrierDustItem = this.game.items.getObjectByID('melvorAoD:Barrier_Dust');
    if (barrierDustItem === undefined) return;
    let qty = Math.max(Math.floor(this.enemy.stats.maxBarrier / numberMultiplier / 20), 1);
    if (rollPercentage(this.getLootDoublingChance(monster))) qty *= 2;
    const autoLooted =
      this.player.modifiers.autoLooting &&
  ...
  dropSignetHalfB(monster) {
    ...
    if (this.player.damageType.id === 'melvorD:Normal' && rollPercentage(chance)) {
      let itemID = 'melvorD:Gold_Topaz_Ring';
      if (this.player.modifiers.allowSignetDrops) itemID = 'melvorD:Signet_Ring_Half_B';
      const item = this.game.items.getObjectByID(itemID);
      if (item === undefined) throw new Error(`Invalid item ID ${itemID}`);
      this.bank.addItem(item, 1, true, true, false, true, `GlobalRare.${item.id}`);
      if (itemID === 'melvorD:Gold_Topaz_Ring') this.game.stats.General.inc(GeneralStats.SignetRingHalvesMissed);
    }
  }
  dropBirthdayPresent() {
    if (!cloudManager.isBirthdayEvent2023Active() || !game.settings.toggleBirthdayEvent) return;
    const chance = 1 / 20;
    if (rollPercentage(chance)) {
      const itemID = 'melvorF:Birthday_Present_Combat';
      const item = this.game.items.getObjectByID(itemID);
      if (item === undefined) throw new Error(`Invalid item ID ${itemID}`);
  ...
  dropEnemyBones(monster) {
    ...
    if (monster.bones !== undefined) {
      ...
      if (item instanceof BoneItem) {
        if (this.player.modifiers.doubleBoneDrops > 0) itemQty *= 2;
      ...
      if (rollPercentage(doubleChance)) itemQty *= 2;
      if (this.game.modifiers.convertBoneDropsIntoCake > 0) {
        const birthdayItem = game.items.getObjectByID('melvorF:Birthday_Cake_Slice');
        if (birthdayItem !== undefined) item = birthdayItem;
      }
      if (item instanceof BoneItem && this.player.modifiers.autoBurying > 0 && this.game.prayer.isUnlocked) {
        ...
        this.player.addPrayerPoints(
          applyModifier(itemQty * this.bank.getPrayerPointsPerBone(item), this.player.modifiers.autoBurying),
        );
      } else {
        const autoLooted =
          this.player.modifiers.autoLooting &&
  ...
  dropEnemyCurrency(monster) {
    monster.currencyDrops.forEach(({ currency, min, max }) => {
      ...
      quantity += this.player.modifiers.getValue('melvorD:flatCurrencyGainFromMonsterDrops', currency.modQuery);
      let modifier = this.player.modifiers.getValue('melvorD:currencyGainFromMonsterDrops', currency.modQuery);
      if (this.onSlayerTask)
        modifier += this.player.modifiers.getValue('melvorD:currencyGainFromSlayerTaskMonsterDrops', currency.modQuery);
      modifier +=
        this.player.modifiers.getValue('melvorD:currencyGainFromMonsterDropsBasedOnDebuffs', currency.modQuery) *
        this.enemy.debuffCount;
      if (quantity > 0) this.addCurrency(currency, quantity, `MonsterKill.${monster.id}`, modifier);
    });
  }
  getCurrencyModifier(currency) {
    let modifier = this.player.modifiers.getValue('melvorD:currencyGain', currency.modQuery);
    modifier += this.player.modifiers.getValue('melvorD:currencyGainFromCombat', currency.modQuery);
    return modifier;
  }
  ...
  rewardSlayerTaskCurrency(category) {
    ...
    const combatLevel =
      (_b = (_a = this.enemy.monster) === null || _a === void 0 ? void 0 : _a.combatLevel) !== null && _b !== void 0
        ? _b
        : 0;
    category.currencyRewards.forEach(({ currency, percent }) => {
      ...
      switch (this.enemy.attackType) {
        case 'melee':
          quantity +=
            this.player.modifiers.getValue(
      ...
      const modifier = this.player.modifiers.getValue('melvorD:currencyGainFromSlayerTasks', currency.modQuery);
  ...
  increaseDungeonProgress(dungeon, monster) {
    ...
    if (this.areaProgress === dungeon.monsters.length) {
      ...
      if (
        !(this.game.settings.autoRestartDungeon && dungeon.id !== 'melvorF:Into_the_Mist') ||
    ...
    if (this.areaProgress === depth.monsters.length) {
        ...
        depth.rewards.forEach((item) => {
          if (
            this.bank.addItem(
              ...
              false,
              true,
              `AbyssDepth.${depth === null || depth === void 0 ? void 0 : depth.id}`,
            )
          ) {
          }
        });
      ...
      if (depth.pet !== undefined) {
        if (depth.fixedPetClears) {
          if (depth.timesCompleted >= depth.pet.weight) this.game.petManager.unlockPet(depth.pet.pet);
        } else {
    ...
    if (this.areaProgress === stronghold.monsters.length) {
      ...
      if (rollPercentage(rewards.chance)) {
        ...
        if (rewards.currencies !== undefined)
          rewards.currencies.forEach(({ currency, quantity }) =>
  ...
  startEvent(event) {
    ...
    combatAreaMenus.openCategory(this.game.combatAreaCategories.getObjectByID('melvorF:SlayerAreas'));
    this.renderQueue.eventMenu = true;
    this.renderEventMenu();
  }
  ...
  fireEventStageCompletionModal(event) {
    ...
    modalBody.append(
      createElement('h5', { text: itemFlavour, className: 'text-info' }),
      createElement('img', { className: 'bank-img', attributes: [['src', item.media]] }),
      createElement('br'),
      createElement('span', { className: 'text-success', text: item.name }),
      createElement('br'),
      descriptionEl,
    );
    addModalToQueue({ title, html: modalBody, allowOutsideClick: false, showConfirmButton: true });
  }
  ...
  showEventPassivesModal() {
    ...
    this.eventPassives.map((passive) => {
      modalBody.append(
        createElement('small', { innerHTML: passive.description, className: 'text-danger' }),
        createElement('br'),
      );
    });
    addModalToQueue({ title: getLangString('BANE_EVENT_4'), html: modalBody });
  }
  showStartEventModal(event) {
    const modalBody = createElement('div');
    modalBody.append(createElement('h5', { text: getLangString('BANE_EVENT_START_TEXT_0') }));
    modalBody.append(createElement('p', { text: getLangString('BANE_EVENT_START_TEXT_1') }));
    if (this.game.currentGamemode.isPermaDeath) {
      modalBody.append(createElement('p', { text: getLangString('BANE_EVENT_START_TEXT_2') }));
    }
    for (let i = 3; i < 9; i++) {
      modalBody.append(createElement('p', { text: getLangString(`BANE_EVENT_START_TEXT_${i}`) }));
    }
    SwalLocale.fire({
  ...
  showStopEventModal() {
    ...
    modalBody.append(createElement('p', { text: getLangString('BANE_EVENT_ARE_YOU_SURE') }));
    SwalLocale.fire({
  ...
  onPassiveSelection(passive) {
    if (!this.eventPassivesBeingSelected.has(passive)) return;
    this.eventPassives.push(passive);
    this.removeAvailablePassive(passive);
    this.eventPassivesBeingSelected.clear();
    if (passive.id === 'melvorF:EventPassive12') this.renderQueue.slayerAreaEffects = true;
    Swal.close();
  }
  removeAvailablePassive(passive) {
    const index = this.availableEventPassives.indexOf(passive);
    if (index === -1) throw new Error('Tried to remove passive that is not available');
    this.availableEventPassives.splice(index, 1);
  }
  increaseEventProgress(event) {
    this.eventProgress++;
    if (this.eventProgress > 0) this.fireEventStageCompletionModal(event);
    if (this.eventProgress === 5) {
  ...
  renderEventAreas() {
    if (this.isEventActive) {
      combatAreaMenus.all.forEach((menu, category) => {
        if (category.id === 'melvorF:SlayerAreas') menu.updateEvent(this.activeEventAreas);
        else menu.updateEvent(new Map());
      });
    } else {
  ...
  isDamageTypeBlockedFromArea(damageType, area) {
    const damageTypeBlocked =
      !area.canEnterWithDamageType(damageType) ||
  ...
  notifyIncorrectDamageTypeForMonster(monster) {
    ...
    const compatibleDamageTypes = game.damageTypes.filter((damageType) => {
      return (
        !this.isDamageTypeBlockedFromMonster(damageType, monster) &&
        damageType.id !== 'melvorF:Pure' &&
        damageType.id !== 'melvorItA:Eternal'
      );
    });
    let html = ``;
  ...
  notifyIncorrectDamageTypeForArea(area) {
    ...
    const compatibleDamageTypes = game.damageTypes.filter((damageType) => {
      return (
        !this.isDamageTypeBlockedFromArea(damageType, area) &&
        damageType.id !== 'melvorF:Pure' &&
        damageType.id !== 'melvorItA:Eternal'
      );
    });
    ...
    html += `<br><br>${getLangString(`THIEVING_DAMAGE_TYPE_NOTICE_1`)}${damageTypeList}`;
    if (setLang === 'en') {
      if (area.id === 'melvorItA:Into_The_Abyss') {
  ...
  selectMonster(monster, area) {
    ...
    if (!this.checkAreaEntryRequirements(area)) return;
    if (this.isCurrentDamageTypeDisallowed(area, true)) return;
    if (!this.checkDamageTypeRequirementsForMonster(monster, true)) return;
  ...
  selectDungeon(dungeon) {
    if (!this.checkAreaEntryRequirements(dungeon)) return;
    if (this.isCurrentDamageTypeDisallowed(dungeon, true)) return;
  ...
  selectAbyssDepth(depth) {
    if (!this.checkAreaEntryRequirements(depth)) return;
    if (this.isCurrentDamageTypeDisallowed(depth, true)) return;
    const canStart = this.preSelection();
    if (canStart) {
  ...
  selectStronghold(stronghold, tier) {
    if (!this.checkAreaEntryRequirements(stronghold) || !this.canFightInStrongholdTier(stronghold, tier)) return;
    if (this.isCurrentDamageTypeDisallowed(stronghold, true)) return;
    const canStart = this.preSelection();
    if (canStart) {
  ...
  selectEventArea(area) {
    ...
    if (areaLength === undefined) throw new Error('Tried to select an event area that is not active!');
    const canStart = this.preSelection();
    if (canStart) {
      this.setCombatArea(area);
      this.areaProgress = 0;
  ...
  stop(fled = true, areaChange = false) {
    ...
    super.stop(fled);
    if (fled && !areaChange) this.loot.lootAll();
  ...
  loadNextEnemy() {
    ...
    if (nextMonster.id === 'melvorF:RandomITM') {
      nextMonster = getRandomArrayElement(this.itmMonsters);
    } else if (nextMonster.id === 'melvorTotH:RandomSpiderLair') {
  ...
  createNewEnemy() {
    var _a;
    if (this.selectedMonster === undefined) throw new Error('Error creating new enemy, no monster is selected.');
  ...
  getPassivesForMonster(monster, area) {
    ...
    const addUnsavedPassive = (passive) => {
      passives.push({ passive, display: true, save: false });
    };
  ...
  computePassivesForEnemy(monster) {
    ...
    const addSavedPassive = (passive) => {
      passives.push({ passive, display: true, save: true });
    };
    if (this.activeEvent !== undefined) {
      this.eventPassives.forEach((passive) => passives.push({ passive, display: false, save: true }));
  ...
  applyAutoCorruption(monster, passives) {
    var _a;
    if (
      !this.game.settings.enablePermaCorruption ||
    ...
    if (this.player.soulPoints >= soulPointCost) {
      ...
      const passive = this.game.combatPassives.getObjectByID('melvorItA:PermanentCorruption');
      if (passive !== undefined) passives.push({ passive, save: true, display: false });
      this.game.stats.Corruption.add(3, soulPointCost);
      this.game.stats.Corruption.inc(2);
    } else {
      if (!loadingOfflineProgress) notifyPlayer(this.game.prayer, getLangString('NOT_ENOUGH_SOUL_POINTS'), 'danger');
    }
  }
  ...
  onPageChange() {
    if (this.game.isGolbinRaid) this.game.golbinRaid.onCombatPageChange();
    else this.onCombatPageChange();
  }
  renderModifierChange() {
    if (this.game.isGolbinRaid) this.game.golbinRaid.renderAutoSwapFood();
    else this.renderAutoSwapFood();
  }
  spawnEnemy() {
    super.spawnEnemy();
  ...
  resetEventState() {
    ...
    this.activeEventAreas.clear();
    this.eventProgress = 0;
    if (this.isActive) this.shouldResetAction = true;
  }
  encode(writer) {
    super.encode(writer);
    ...
    if (this.activeEvent !== undefined) writer.writeNamespacedObject(this.activeEvent);
  ...
  decode(reader, version) {
    ...
    if (reader.getBoolean()) {
      ...
      if (typeof selectedArea === 'string') this.shouldResetAction = true;
      else this.selectedArea = selectedArea;
    }
    this.areaProgress = reader.getUint32();
    if (reader.getBoolean()) {
      const selectedMonster = reader.getNamespacedObject(this.game.monsters);
      if (typeof selectedMonster === 'string') this.shouldResetAction = true;
      else this.selectedMonster = selectedMonster;
    }
    ...
    if (reader.getBoolean()) {
      ...
      if (typeof activeEvent === 'string') this.shouldResetEvent = true;
      else this.activeEvent = activeEvent;
    }
    ...
    this.activeEventAreas = reader.getMap(
      (reader) => {
    ...
    this.dungeonCompletion = reader.getMap(
      (reader) => {
        const dungeon = reader.getNamespacedObject(this.game.dungeons);
        if (typeof dungeon === 'string') {
          if (dungeon.startsWith('melvor')) return this.game.dungeons.getDummyObject(dungeon, DummyDungeon, this.game);
          else return undefined;
        } else return dungeon;
      },
      (reader) => reader.getUint32(),
    );
    if (version >= 100) this.strongholdTier = Stronghold.TierIDs[reader.getUint8()];
    if (version >= 102 && version < 104) reader.skipArrayBytes(2);
    if (this.activeEvent !== undefined) this.computeAvailableEventPassives(this.activeEvent);
    if (this.shouldResetEvent) this.resetEventState();
  ...
  deserialize(reader, version, idMap) {
    ...
    if (version >= 7) {
      ...
      if (eventActive) {
        ...
        if (this.activeEvent === undefined) this.shouldResetEvent = true;
      }
      ...
      if (version < 9) {
        ...
        oldPassiveIDs.forEach((passiveID) => {
          if (passiveID >= 31) passiveID--;
          if (passiveID !== 31) eventPassiveIDs.push(passiveID);
        });
      }
      eventPassiveIDs.forEach((passiveID) => {
        const passive = this.game.combatPassives.getObjectByID(idMap.combatPassives[passiveID]);
        if (passive !== undefined) this.eventPassives.push(passive);
      });
      ...
      if (version < 9) {
        ...
        oldPassivesSelected.forEach((passiveID) => {
          if (passiveID >= 31) passiveID--;
          if (passiveID !== 31) selectedPassiveIDs.push(passiveID);
        });
      }
      selectedPassiveIDs.forEach((passiveID) => {
        const passive = this.game.combatPassives.getObjectByID(idMap.combatPassives[passiveID]);
        if (passive !== undefined) this.eventPassivesBeingSelected.add(passive);
      });
      ...
      for (let i = 0; i < numAreas; i++) {
        ...
        if (slayerArea === undefined) this.shouldResetEvent = true;
        const monsterCount = reader.getNumber();
        if (slayerArea !== undefined) this.activeEventAreas.set(slayerArea, monsterCount);
      }
    }
    if (version >= 8) {
    ...
    if (this.activeEvent !== undefined) {
      if (version < 13) {
        ...
        this.eventPassives.forEach((passive) => {
          if (!newPassives.includes(passive)) newPassives.push(passive);
        });
  ...
  convertFromOldSaveFormat(saveGame, idMap) {
    ...
    this.slayerTask.convertFromOldSaveFormat(
      (_a = saveGame.slayerTask) !== null && _a !== void 0 ? _a : [],
      (_b = saveGame.slayerTaskCompletion) !== null && _b !== void 0 ? _b : defaultSaveValues.slayerTaskCompletion,
      idMap,
    );
  }
  convertDungeonCompletion(dungeonCompletion, idMap) {
    dungeonCompletion.forEach((count, oldDungeonID) => {
      ...
      if (dungeon === undefined) dungeon = this.game.dungeons.getDummyObject(newID, DummyDungeon, this.game);
      this.dungeonCompletion.set(dungeon, count);
    });
  }
  getCombatStatsLog() {
    return {
      player: { stats: this.player.stats.getValueTable(), modifiers: this.player.modifiers.getLog() },
      enemy: { stats: this.enemy.stats.getValueTable(), modifiers: this.enemy.modifiers.getLog() },
    };
  }
  saveStats() {
  ...
  compareSavedStats() {
    ...
    if (oldStats !== null) this.compareCombatStatLogs(oldStats);
  }
  ...
  testInitializationStatParity() {
    ...
    if (failed) this.game.stopMainLoop();
  }
}
class Timer {
  ...
  start(time, offsetByTick = false) {
    ...
    if (offsetByTick) ticks++;
    if (ticks < 1) throw new Error(`Tried to start timer: ${this.type} with invalid tick amount: ${ticks}`);
    this.active = true;
  ...
  get progress() {
    if (this.active) return (this._maxTicks - this._ticksLeft) / this._maxTicks;
    return 0;
  }
  encode(writer) {
